...
void G_SetStats(edict_t * ent)
{
	...
	if (power_armor_type) {
		cells = ent->client->pers.inventory[ITEM_INDEX(Fdi_CELLS	/* FindItem
																	   ("cells")
																	 */ )];
	...
	// RAFAEL
		...
		ent->client->ps.stats[STAT_TIMER_ICON] =
			gi.imageindex("p_quadfire");
		ent->client->ps.stats[STAT_TIMER] =
			(ent->client->quadfire_framenum - level.framenum) / 10;
	} else if (ent->client->invincible_framenum > level.framenum) {
		ent->client->ps.stats[STAT_TIMER_ICON] =
			gi.imageindex("p_invulnerability");
		ent->client->ps.stats[STAT_TIMER] =
...
//ponpoko
	if (ent->client->zc.aiming == 1) {
		...
		if (ent->client->zc.lockon)
			ent->client->ps.stats[STAT_SIGHT_PIC] =
				gi.imageindex("zsight_l1");
		else
			ent->client->ps.stats[STAT_SIGHT_PIC] =
				gi.imageindex("zsight_l0");
	} else
		ent->client->ps.stats[STAT_SIGHT_PIC] = 0;	// gi.imageindex
													// ("i_help"/*"zsight"*/);;
//ponpoko
