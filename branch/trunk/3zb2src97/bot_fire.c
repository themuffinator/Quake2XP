	...
	case WEAP_RAILGUN:
		if (target != ent->client->zc.last_target) {
			if (target->svflags & SVF_MONSTER) {
				VectorSubtract(target->s.old_origin, target->s.origin,
							   targaim);
			} else {
			...
			VectorMA(target->s.origin,
					 random() * aim * AIMING_POSGAP * random(), targaim,
					 targaim);
		} else {
			VectorSubtract(ent->client->zc.last_pos, target->s.origin,
						   targaim);
//			VectorScale (targaim, vec_t scale, vec3_t out)
			VectorMA(target->s.origin,
					 aim * /* VectorLength(targaim)* */ random(), targaim,
					 targaim);
		}
	...
	case WEAP_CHAINGUN:
		if (target != ent->client->zc.last_target) {
			...
			VectorNormalize(targaim);
			VectorMA(target->s.origin, random() * aim * AIMING_POSGAP,
					 targaim, targaim);
		} else {
			VectorSubtract(ent->client->zc.last_pos, target->s.origin,
						   targaim);
			VectorMA(target->s.origin,
					 random() * aim /** VectorLength(targaim)*/ , targaim,
					 targaim);
		}
		VectorSubtract(targaim, ent->s.origin, targaim);
	...
	case WEAP_BOOMER:
		if (target != ent->client->zc.last_target) {
			if (target->svflags & SVF_MONSTER) {
				VectorSubtract(target->s.origin, target->s.old_origin,
							   targaim);
			} else {
				VectorCopy(target->velocity, targaim);
				targaim[0] *= 32;
			...
			VectorNormalize(targaim);
			VectorMA(target->s.origin, (11 - aim) * dist / 25, targaim,
					 targaim);
		} else {
			VectorSubtract(target->s.origin, ent->client->zc.last_pos,
						   targaim);
			targaim[2] /= 2;
			VectorMA(target->s.origin, -aim * random() + dist / 75,
					 targaim, targaim);
		}
		rs_trace =
			gi.trace(target->s.origin, NULL, NULL, targaim, target,
					 MASK_SHOT);
		...
		if (weapon == WEAP_GRENADELAUNCHER
			...
			if (targaim[2] < (ent->s.origin[2] + JumpMax)) {
				...
				rs_trace =
					gi.trace(v, NULL, NULL, targaim, ent, MASK_SHOT);
	...
	case WEAP_HYPERBLASTER:
		if (target != ent->client->zc.last_target) {
			...
			VectorNormalize(targaim);
			VectorMA(target->s.origin, (11 - aim) * dist / 100, targaim,
					 targaim);
		} else {
			VectorSubtract(target->s.origin, ent->client->zc.last_pos,
						   targaim);
			targaim[2] /= 2;
			VectorMA(target->s.origin, -aim + dist / 115, targaim,
					 targaim);
		}
		rs_trace =
			gi.trace(target->s.origin, NULL, NULL, targaim, target,
...
//  Use BFG
//------------------------------------------------------------
qboolean B_UseBfg(edict_t * ent, edict_t * target, int enewep, float aim,
...
//  Use Hyper Blaster
//------------------------------------------------------------
qboolean B_UseHyperBlaster(edict_t * ent, edict_t * target, int enewep,
...
//  Use Phalanx
//------------------------------------------------------------
qboolean B_UsePhalanx(edict_t * ent, edict_t * target, int enewep,
					  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Rocket
//------------------------------------------------------------
qboolean B_UseRocket(edict_t * ent, edict_t * target, int enewep,
					 float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Boomer
//------------------------------------------------------------
qboolean B_UseBoomer(edict_t * ent, edict_t * target, int enewep,
					 float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Railgun
//------------------------------------------------------------
qboolean B_UseRailgun(edict_t * ent, edict_t * target, int enewep,
					  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//------------------------------------------------------------
qboolean B_UseGrenadeLauncher(edict_t * ent, edict_t * target, int enewep,
							  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Chain Gun
//------------------------------------------------------------
qboolean B_UseChainGun(edict_t * ent, edict_t * target, int enewep,
					   float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Machine Gun
//------------------------------------------------------------
qboolean B_UseMachineGun(edict_t * ent, edict_t * target, int enewep,
						 float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use S-Shotgun
//------------------------------------------------------------
qboolean B_UseSuperShotgun(edict_t * ent, edict_t * target, int enewep,
						   float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Shotgun
//------------------------------------------------------------
qboolean B_UseShotgun(edict_t * ent, edict_t * target, int enewep,
					  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Hand Grenade
//------------------------------------------------------------
qboolean B_UseHandGrenade(edict_t * ent, edict_t * target, int enewep,
						  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Trap
//------------------------------------------------------------
qboolean B_UseTrap(edict_t * ent, edict_t * target, int enewep, float aim,
				   float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
...
//  Use Blaster
//------------------------------------------------------------
qboolean B_UseBlaster(edict_t * ent, edict_t * target, int enewep,
					  float aim, float distance, int skill)
{
	int k, mywep;
	zgcl_t *zc;
				   ...
				   float distance, int skill)
{
	...
	if (zc->battlemode & FIRE_ESTIMATE) {
		...
		if (distance > 100 && distance < 400
			...
			if (B_UseGrenadeLauncher
				(ent, target, enewep, aim, distance, skill))
				k = true;
		}
		// Hand Grenade
		if (distance < 1200) {
				   ...
				   float distance, int skill)
{
	...
	if (Bot[ent->client->zc.botindex].param[BOP_DODGE]
		...
		if (rs_trace.ent == ent) {
			...
			} else if (rs_trace.endpos[2] <
					   (ent->s.origin[2] + JumpMax - 24)) {
				if (zc->route_trace) {
					if (Bot_Fall(ent, ent->s.origin, 0))
						trace_priority = TRP_MOVEKEEP;;
				} else {
	...
	if (zc->battlemode & FIRE_STAYFIRE) {
		if (--zc->battlecount > 0) {
			...
			if (!(zc->battlemode & FIRE_SHIFT))
				trace_priority = TRP_ALLKEEP;
			if (Bot_traceS(ent, target)
				|| mywep == WEAP_BFG || mywep == WEAP_GRENADELAUNCHER)
				client->buttons |= BUTTON_ATTACK;
			return;
		}
		zc->battlemode = 0;
...
//			trace_priority = TRP_ALLKEEP;
	...
	if (!(client->zc.zccmbstt & CTS_ENEM_NSEE)
		...
		if (k = CanUsewep(ent, WEAP_GRENADELAUNCHER)) {
			...
			Get_AimAngle(ent, aim, distance, mywep);
			if ((target->client->weaponstate == WEAPON_FIRING
				 && ent->groundentity) || (zc->zcstate & STS_WAITSMASK2))
				ent->client->ps.pmove.pm_flags |= PMF_DUCKED;
			client->buttons |= BUTTON_ATTACK;
			trace_priority = TRP_ANGLEKEEP;
			return;
...
//      mywep = Get_KindWeapon(client->pers.weapon);
	...
	if ((FFlg[skill] & FIRE_REFUGE)
		...
		if (fabs(v[2] - ent->s.origin[2]) < JumpMax && j) {
			mywep = Get_KindWeapon(client->pers.weapon);
			if (mywep == WEAP_GRENADELAUNCHER
				|| mywep == WEAP_ROCKETLAUNCHER || mywep == WEAP_PHALANX) {
	...
	for (i = 0; i < 3; i++) {
		...
		case WEAP_HYPERBLASTER:
			if (distance < 1200) {
				if (B_UseHyperBlaster
					(ent, target, enewep, aim, distance, skill))
					goto FIRED;
			}
			break;
		case WEAP_PHALANX:
			if (distance > 100 && distance < 1200	/* || mywep ==
													   WEAP_PHALANX */ ) {
				if (B_UsePhalanx
					(ent, target, enewep, aim, distance, skill))
					goto FIRED;
	...
	// Phalanx
	if ((distance > 100
		 && distance < 1200) /* || mywep == WEAP_PHALANX */ ) {
		if (B_UsePhalanx(ent, target, enewep, aim, distance, skill))
			goto FIRED;
	}
	// Rocket
  ...
  FIRED:
	...
	if (zc->battlemode == FIRE_CHIKEN) {
		if (--zc->battlesubcnt > 0 && ent->groundentity
			&& ent->waterlevel < 2) {
			f = target->s.angles[YAW] - ent->s.angles[YAW];
