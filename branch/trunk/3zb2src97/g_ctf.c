...
typedef struct ctfgame_s {
	int team1, team2;
	int total1, total2;			// these are only set when going into
								// intermission!
	float last_flag_capture;
	int last_capture_team;
} ctfgame_t;
//PON
qboolean bots_moveok(edict_t * ent, float ryaw, vec3_t pos, float dist,
					 float *bottom);
//PON
ctfgame_t ctfgame;
...
// picked up item
	...
	"	xv	0 "
	"	pic 7 "
	"	xv	26 " "	yb	-42 " "	stat_string 8 " "	yb	-50 " "endif "
// timer
...
//  frags
	"xr	-50 " "yt 2 " "num 3 14 "
//tech
	"yb -129 " "if 26 " "xr -26 " "pic 26 " "endif "
// red team
	"yb -102 " "if 17 " "xr -26 " "pic 17 " "endif " "xr -62 " "num 2 18 "
//joined overlay
...
// blue team
	...
	"pic 19 "
	"endif "
	"xr -62 " "num 2 20 " "if 23 " "yb -77 " "xr -28 " "pic 23 " "endif "
// have flag graph
	"if 21 " "yt 26 " "xr -24 " "pic 21 " "endif "
// id view state
	"if 27 "
	"xv 0 "
	"yb -58 " "string \"Viewing\" " "xv 64 " "stat_string 27 " "endif "
//sight
	"if 31 " "   xv 96 " "   yv 56 " "   pic 31 " "endif";
...
order.
*/
void CTFFragBonuses(edict_t * targ, edict_t * inflictor,
					edict_t * attacker)
{
	int i;
	edict_t *ent;
...
//PONKO
	if (attacker) {
		...
		if (VectorLength(v1) < 300
			&& attacker->client && !(attacker->deadflag)
	...
	if (ctf_team == other->client->resp.ctf_team) {
		...
		if (!(ent->spawnflags & DROPPED_ITEM)) {
			...
			if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
				...
				other->client->pers.
					inventory[ITEM_INDEX(enemy_flag_item)] = 0;
				...
				gi.sound(ent, CHAN_RELIABLE + CHAN_NO_PHS_ADD + CHAN_VOICE,
						 gi.soundindex("ctf/flagcap.wav"), 1, ATTN_NONE,
						 0);
...
void SetCTFStats(edict_t * ent)
{
	...
	// if during intermission, we must blink the team header of the
	// winning team
	if (level.intermissiontime && (level.framenum & 8)) {	// blink 1/8th
															// second
...
void CTFResetGrapple(edict_t * self)
{
	...
	if (self->owner->client->ctf_grapple) {
		...
		gi.sound(self->owner, CHAN_RELIABLE + CHAN_WEAPON,
				 gi.soundindex("weapons/grapple/grreset.wav"), volume,
				 ATTN_NORM, 0);
		cl = self->owner->client;
		cl->ctf_grapple = NULL;
		cl->ctf_grapplereleasetime = level.time;
		cl->ctf_grapplestate = CTF_GRAPPLE_STATE_FLY;	// we're firing,
														// not on hook
...
void CTFGrappleTouch(edict_t * self, edict_t * other, cplane_t * plane,
					 csurface_t * surf)
{
	short i;
	float volume = 1.0;
	...
	self->owner->client->ctf_grapplestate = CTF_GRAPPLE_STATE_PULL;	// we're
																	// on
																	// hook
	self->enemy = other;
...
//PON-CTF
	if (chedit->value && self->owner == &g_edicts[1]) {
		...
		if (++CurrentIndex < MAXNODES) {
			gi.bprintf(PRINT_HIGH,
					   "Grapple has been hook.Last %i pod(s).\n",
					   MAXNODES - CurrentIndex);
			memset(&Route[CurrentIndex], 0, sizeof(route_t));	// initialize
...
//PON-CTF
	...
	gi.sound(self->owner, CHAN_RELIABLE + CHAN_WEAPON,
			 gi.soundindex("weapons/grapple/grpull.wav"), volume,
			 ATTN_NORM, 0);
...
void CTFGrappleDrawCable(edict_t * self)
{
	...
	if (1 /* !(self->owner->svflags & SVF_MONSTER) */ ) {
		...
		if (self->owner->maxs[2] >= 32)
			start[2] = self->owner->s.origin[2] + 16;
		else
			start[2] = self->owner->s.origin[2] - 12;
	}
...
/*	if(!(self->owner->svflags & SVF_MONSTER))
	...
	if (self->enemy) {
		...
		if (self->enemy->takedamage &&
			...
			gi.sound(self, CHAN_WEAPON,
					 gi.soundindex("weapons/grapple/grhurt.wav"), volume,
					 ATTN_NORM, 0);
		}
	...
	if (self->owner->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY) {
		...
		if (self->owner->client->ctf_grapplestate == CTF_GRAPPLE_STATE_PULL
			...
			gi.sound(self->owner, CHAN_RELIABLE + CHAN_WEAPON,
					 gi.soundindex("weapons/grapple/grhang.wav"), volume,
					 ATTN_NORM, 0);
...
void CTFFireGrapple(edict_t * self, vec3_t start, vec3_t dir, int damage,
...
//  grapple->think = CTFGrappleThink;
	...
	self->client->ctf_grapplestate = CTF_GRAPPLE_STATE_FLY;	// we're
															// firing, not
															// on hook
	gi.linkentity(grapple);

	tr = gi.trace(self->s.origin, NULL, NULL, grapple->s.origin, grapple,
				  MASK_SHOT);
	if (tr.fraction < 1.0) {
...
//PON-CTF
	if (chedit->value && self == &g_edicts[1]) {
		...
		if (++CurrentIndex < MAXNODES) {
			gi.bprintf(PRINT_HIGH,
					   "Grapple has been shoot.Last %i pod(s).\n",
					   MAXNODES - CurrentIndex);
			memset(&Route[CurrentIndex], 0, sizeof(route_t));	// initialize
			Route[CurrentIndex].index = Route[CurrentIndex - 1].index + 1;
		}
...
//  VectorSet(offset, 24, 16, ent->viewheight-8+2);
	...
	gi.sound(ent, CHAN_RELIABLE + CHAN_WEAPON,
			 gi.soundindex("weapons/grapple/grfire.wav"), volume,
			 ATTN_NORM, 0);
...
void CTFWeapon_Grapple(edict_t * ent)
{
	...
	if (!(ent->client->buttons & BUTTON_ATTACK) &&
		...
		if (ent->svflags & SVF_MONSTER) {
			if (ent->waterlevel || i == CTF_GRAPPLE_STATE_HANG)
			...
			if (i == CTF_GRAPPLE_STATE_PULL) {
				...
				ent->moveinfo.speed =
					(VectorLength(v) * FRAMETIME) / MOVE_SPD_RUN;
				ent->velocity[0] = 0;
				ent->velocity[1] = 0;
		...
		*entry = 0;
		...
		if (i < total[0]) {
			...
			sprintf(entry + strlen(entry),
					...
					cl->resp.score,
					(cl->ping > 999) ? 999 : cl->ping, cl->pers.netname);
...
int CTFApplyResistance(edict_t * ent, int dmg)
{
	...
	if (dmg && tech && ent->client
		&& ent->client->pers.inventory[ITEM_INDEX(tech)]) {
		// make noise
		gi.sound(ent, CHAN_VOICE, gi.soundindex("ctf/tech1.wav"), volume,
				 ATTN_NORM, 0);
		return dmg / 2;
	}
	return dmg;
	...
	if (tech && ent->client &&
		...
		if (ent->client->ctf_techsndtime < level.time) {
			...
			if (ent->client->quad_framenum > level.framenum)
				gi.sound(ent, CHAN_VOICE, gi.soundindex("ctf/tech2x.wav"),
						 volume, ATTN_NORM, 0);
			else
...
void CTFApplyHasteSound(edict_t * ent)
{
	...
	if (tech && ent->client &&
		...
		ent->client->ctf_techsndtime = level.time + 1;
		gi.sound(ent, CHAN_VOICE, gi.soundindex("ctf/tech3.wav"), volume,
				 ATTN_NORM, 0);
	}
}
...
void CTFApplyRegeneration(edict_t * ent)
{
	...
	if (tech && client->pers.inventory[ITEM_INDEX(tech)]) {
		...
		if (noise && ent->client->ctf_techsndtime < level.time) {
			ent->client->ctf_techsndtime = level.time + 1;
			gi.sound(ent, CHAN_VOICE, gi.soundindex("ctf/tech4.wav"),
					 volume, ATTN_NORM, 0);
		}
	}
}
...
static void CTFSay_Team_Location(edict_t * who, char *buf)
{
	...
	while ((what = G_Find(what, FOFS(classname), hot->classname)) != NULL) {
		...
		if ((flag1 =
			 G_Find(NULL, FOFS(classname), "item_flag_team1")) != NULL
			&& (flag2 =
				G_Find(NULL, FOFS(classname),
					   "item_flag_team2")) != NULL) {
...
static void CTFSay_Team_Armor(edict_t * who, char *buf)
{
	...
	*buf = 0;
	...
	if (power_armor_type) {
		cells = who->client->pers.inventory[ITEM_INDEX(Fdi_CELLS	/* FindItem
																	   ("cells")
																	 */ )];
		if (cells)
...
static void old_teleporter_touch(edict_t * self, edict_t * other,
								 cplane_t * plane, csurface_t * surf)
{
	edict_t *dest;
	int i;
...
//PONKO
	...
	if (!ctf->value)
		sprintf(name, ".\\%s\\chdtm\\%s.chn", gamepath->string,
				level.mapname);
	else
		sprintf(name, ".\\%s\\chctf\\%s.chf", gamepath->string,
				level.mapname);

	fpout = fopen(name, "rb");
	if (fpout == NULL) {
		if (!ctf->value)
			gi.dprintf("Chaining: file %s.chn not found.\n",
					   level.mapname);
		else
			gi.dprintf("Chaining: file %s.chf not found.\n",
					   level.mapname);
	} else {
...
//gi.bprintf(PRINT_HIGH,"I am carrierY!!\n");
			if (e->client->resp.ctf_team == CTF_TEAM1) {
				mate1++;
				if (e->client->pers.
					inventory[ITEM_INDEX(FindItem("Blue Flag"))]) {
				...
				mate2++;
				if (e->client->pers.
					inventory[ITEM_INDEX(FindItem("Red Flag"))]) {
					client->zc.ctfstate = CTFS_CARRIER;
				}
				if (1 /* e->svflags & SVF_MONSTER */ ) {
					if (client->zc.ctfstate == CTFS_OFFENCER
