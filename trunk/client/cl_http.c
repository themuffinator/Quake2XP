...
static void CL_StartHTTPDownload (dlqueue_t *entry, dlhandle_t *dl)
{
	...
	curl_easy_setopt (dl->curl, CURLOPT_PROXY, cl_httpProxy->string);
...
void CL_InitHTTPDownloads (void)
{
	...
	Com_Printf("====== " S_COLOR_YELLOW "cURL Initialization" S_COLOR_WHITE " ======\n\n");
	Com_Printf ("%s: " S_COLOR_GREEN "ok.\n\n", curl_version());
	Com_Printf("=================================\n\n");
}
...
qboolean CL_QueueHTTPDownload (const char *quakePath)
{
	...
	if (!cls.downloadServer[0] || abortDownloads || !cl_httpDownloads->integer)
		return qfalse;

	needList = qfalse;

	// first download queued, so we want the mod filelist
	if (!cls.downloadQueue.next && cl_httpFileLists->integer)
		needList = qtrue;
	...
	strncpy (q->quakePath, quakePath, sizeof(q->quakePath)-1);
	...
	if (cl_httpFileLists->integer && len > 4 && !Q_stricmp (quakePath + len - 4, ".bsp"))
	{
...
static void CL_CheckAndQueueDownload (char *path)
{
	...
	if (!pak && strcmp (ext, "pcx") && strcmp (ext, "wal") && strcmp (ext, "wav") && strcmp (ext, "md2") && strcmp(ext, "md3") &&
		strcmp (ext, "sp2") && strcmp (ext, "tga") && strcmp (ext, "png") && strcmp (ext, "jpg") && strcmp(ext, "dds") &&
...
static void CL_FinishHTTPDownload (void)
{
	...
	do
	{
		...
		if (isFile)
		{
			...
			j = strlen (tempName);
			if ((!strcmp(tempName + j - 4, ".pkx")) || (!strcmp(tempName + j - 4, ".pak")))
			{
				if (!strcmp(tempName + j - 4, ".pkx"))
					FS_AddPkxFile(tempName);
				else
...
void CL_RunHTTPDownloads (void)
{
	...
	if (pendingCount && abortDownloads == HTTPDL_ABORT_NONE &&
		!downloading_pak && handleCount < cl_httpMaxConnections->integer)
		CL_StartNextHTTPDownload ();
