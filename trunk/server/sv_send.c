				   ...
				   float attenuation, float timeofs)
{
	...
	if (attenuation < 0 || attenuation > 4)
		Com_Error(ERR_FATAL, "SV_StartSound: attenuation = %f",
				  attenuation);
...
//      Com_Error (ERR_FATAL, "SV_StartSound: channel = %i", channel);
	...
	if (!origin) {
		...
		if (entity->solid == SOLID_BSP) {
			for (i = 0; i < 3; i++)
				origin_v[i] =
					entity->s.origin[i] + 0.5 * (entity->mins[i] +
												 entity->maxs[i]);
		} else {
...
qboolean SV_SendClientDatagram(client_t * client)
{
	...
	if (msg.overflowed) {		// must have room left for the packet
								// header
...
void SV_SendClientMessages(void)
{
	...
	if (sv.state == ss_demo && sv.demofile.f) {
		if (sv_paused->value)
			...
			r = fread(&msglen, 4, 1, sv.demofile.f);
			if (r != 1) {
				SV_DemoCompleted();
				return;
			}
			...
			r = fread(msgbuf, msglen, 1, sv.demofile.f);
			if (r != 1) {
				SV_DemoCompleted();
				return;
			}
	...
	for (i = 0, c = svs.clients; i < maxclients->value; i++, c++) {
		...
		if (sv.state == ss_cinematic
			|| sv.state == ss_demo || sv.state == ss_pic)
