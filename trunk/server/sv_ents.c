...
{
	byte bits[16];				// [modelindex] [48 bits] xyz p y 12 12 12 
	// 8 8 [entitynum] [e2]
	int n, i;
	edict_t *ent;
	int x, y, z, p, yaw;
...
void SV_EmitPacketEntities (client_frame_t * from, client_frame_t * to,
	...
	while (newindex < to->num_entities || oldindex < from_num_entities) {
		...
		if (newindex >= to->num_entities)
			...
			newent =
				&svs.client_entities[(to->first_entity + newindex) %
		...
		if (oldindex >= from_num_entities)
			...
			oldent =
				&svs.client_entities[(from->first_entity + oldindex) %
				svs.num_client_entities];
...
void SV_WriteFrameToClient (client_t * client, sizebuf_t * msg) {
	...
	else if (sv.framenum - client->lastframe >= (UPDATE_BACKUP - 3)) {	// client 
		// hasn't 
		// gotten 
		// a 
		// good 
...
void SV_BuildClientFrame (client_t * client) {
	...
	for (i = 0; i < 3; i++)
		org[i] =
		clent->client->ps.pmove.origin[i] * 0.125 +
	...
	for (e = 1; e < ge->num_edicts; e++) {
		...
		// willow: i do not care of areas and PVS for specific entities.
		if (!(ent->s.sound && sv_unlimited_ambient->value))
			// ignore if not touching a PV leaf
		if (ent != clent) {
			// check area
			if (!CM_AreasConnected (clientarea, ent->areanum)) {	// doors 
				// can 
				// legally 
				// straddle 
				// two 
		...
		state =
			&svs.client_entities[svs.next_client_entities %
			svs.num_client_entities];
		if (ent->s.number != e) {
