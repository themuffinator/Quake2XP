...
//  if (ent->solid == SOLID_BBOX)
	{							// assume that x/y are equal and symetric
		...
		ent->s.solid = 31;		// a solid_bbox will never create this
								// value
	} else
		ent->s.solid = 0;

	// set the abs box
	if (ent->solid == SOLID_BSP && (ent->s.angles[0] || ent->s.angles[1] || ent->s.angles[2])) {	// expand 
																									// for 
																									// rotation
		float max, v;
		int i;
		...
		for (i = 0; i < num_leafs; i++) {
			...
			for (j = 0; j < i; j++)
				if (clusters[j] == clusters[i])
					break;
			if (j == i) {
				if (ent->num_clusters == MAX_ENT_CLUSTERS) {	// assume
																// we
																// missed
																// some
																// leafs,
...
void SV_ClipMoveToEntities(moveclip_t * clip)
{
	...
	num =
		SV_AreaEdicts(clip->boxmins, clip->boxmaxs, touchlist, MAX_EDICTS,
					  AREA_SOLID);
