...
cvar_t *sv_noreload;			// don't reload level state when
// reentering

cvar_t *maxclients;				// FIXME: rename sv_maxclients
cvar_t *sv_unlimited_ambient;	// disables entities clipping with sound
// flag active
cvar_t *sv_showclamp;
...
void SV_ReadPackets (void) {
	...
	while (NET_GetPacket (NS_SERVER, &net_from, &net_message)) {
		...
		for (i = 0, cl = svs.clients; i < maxclients->value; i++, cl++) {
			...
			if (Netchan_Process (&cl->netchan, &net_message)) {	// this is 
				// a
				// valid,
				// sequenced 
				// packet, 
...
void Master_Heartbeat (void) {
	...
	// pgm post3.19 change, cvar pointer not validated before
	// dereferencing
...
void SV_Init (void) {
	...
	hostname =
		Cvar_Get ("hostname", "noname", CVAR_SERVERINFO | CVAR_ARCHIVE);
	...
	sv_unlimited_ambient =
		Cvar_Get ("sv_unlimited_ambient", "0", 0);

	sv_solidcorpse = Cvar_Get ("sv_solidcorpse", "0", CVAR_ARCHIVE);
