...
void SV_SpawnServer (char *server, char *spawnpoint,
	...
	if (Cvar_VariableValue ("deathmatch")) {
		sprintf (sv.configstrings[CS_AIRACCEL], "%g",
			sv_airaccelerate->value);
...
void SV_InitGame (void) {
	...
	Com_sprintf (idmaster, sizeof(idmaster), "192.246.40.37:%i",
		PORT_MASTER);
...
Kirk Barnes - cut off cinematics
======================
*/
#if 0
void SV_Map (qboolean attractloop, char *levelstring, qboolean loadgame) {
	...
	if (ch) {
		...
		if (*a == '*')
			a++;
		Cvar_Set("nextserver", va("gamemap \"%s\"", a));
	...
	if (level[0] == '*')		// cut intro cinematic
	{
		strcpy(level, level + 10);
	}

	ch = strstr(level, "+");	// cut ingame cinematic
	if (ch) {
		if (strstr(level, "victory.pcx"))	// detect end screen
		{
			*ch = 0;
			strcpy(level, level + 8);
		}
		/* else if(strstr(level, "xout.cin")) //xatrix end screen { *ch = 
		   0; strcpy (level, level+9); } */
		else {
			*ch = 0;
			strcpy(level, level + 11);
		}
	}
