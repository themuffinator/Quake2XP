	...
	while (sv_client->netchan.message.cursize < MAX_MSGLEN * 0.5
		...
		if (base->modelindex || base->sound || base->effects) {
			...
			MSG_WriteDeltaEntity (&nullstate, base,
				&sv_client->netchan.message, qtrue, qtrue);
		}
		start++;
	}
...
void SV_BeginDownload_f (void) {
	...
	if (strstr (name, "..") || !allow_download->value
		...
		&& !allow_download_players->value)
		// now models
		|| (strncmp (name, "models/", 6) == 0
...
void SV_Nextserver (void) {
	...
	if (sv.state == ss_game
		...
		return;					// can't nextserver while playing a normal 
	// game
