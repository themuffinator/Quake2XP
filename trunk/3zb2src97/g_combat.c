...
//      targ->svflags |= SVF_DEADMONSTER;   // now treat as a different content type
	...
	if (targ->movetype == MOVETYPE_PUSH || targ->movetype == MOVETYPE_STOP || targ->movetype == MOVETYPE_NONE) {	// doors,
																													// triggers,
																													// etc
...
static int CheckArmor(edict_t * ent, vec3_t point, vec3_t normal,
			  ...
			  int knockback, int dflags, int mod)
{
	...
	if (mod == MOD_CRUSH) {
		...
		if ((targ->svflags & SVF_MONSTER) && targ->client) {
			if ((targ->client->zc.waitin_obj == inflictor
				 && targ->client->zc.zcstate)
...
//              gi.bprintf(PRINT_HIGH,"MOOOOOOOOOOOOOOOOOOOO\n");
	...
	if ((targ != attacker)
		...
		if (OnSameTeam(targ, attacker)) {
			...
			if (attacker->client)
				targ->client->zc.first_target = attacker;
		}
	}
	meansOfDeath = mod;
...
// figure momentum add
	if (!(dflags & DAMAGE_NO_KNOCKBACK)) {
		if ((knockback) && (targ->movetype != MOVETYPE_NONE)
			...
			if (targ->client && attacker == targ)
				VectorScale(dir, 1600.0 * (float) knockback / mass, kvel);	// the
																			// rocket
																			// jump
																			// hack...
			else
	...
	if ((client && client->invincible_framenum > level.framenum)
		...
		if (targ->pain_debounce_time < level.time) {
			gi.sound(targ, CHAN_ITEM, gi.soundindex("items/protect3.wav"),
					 1, ATTN_NORM, 0);
...
// do the damage
	if (take) {
		if ((targ->svflags & SVF_MONSTER) || (client)) {
			...
			if (client && (targ->svflags & SVF_MONSTER) && attacker) {
				if (client->zc.battlemode & FIRE_CHIKEN)
					client->zc.battlemode &= ~FIRE_CHIKEN;
				...
				if (mod == MOD_RAILGUN
					...
					|| mod == MOD_BLASTER
					|| mod == MOD_RIPPER
					|| mod == MOD_HYPERBLASTER || mod == MOD_PHALANX) {
					if (attacker->client
						&& (9 * random() <
							Bot[client->zc.botindex].param[BOP_REACTION])
						&& !client->zc.first_target) {
						if (!OnSameTeam(targ, attacker))
							client->zc.first_target = attacker;
					}
				}
			}
		} else
	...
	// the total will be turned into screen dFactor and view angle kicks
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL) {
		...
		if (points > 0) {
			if (CanDamage(ent, inflictor)) {
				...
				T_Damage(ent, inflictor, attacker, dir,
						 inflictor->s.origin, vec3_origin, (int) points,
