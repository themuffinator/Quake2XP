...
void AngleMove_Begin(edict_t * ent)
{
	...
	if (ent->moveinfo.state == STATE_UP)
		...
		VectorSubtract(ent->moveinfo.start_angles, ent->s.angles,
					   destdelta);
...
void plat_CalcAcceleratedMove(moveinfo_t * moveinfo)
{
	...
	if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0) {
		...
		moveinfo->move_speed =
			(-2 +
			 sqrt(4 -
				  4 * f * (-2 * moveinfo->remaining_distance))) / (2 * f);
		decel_dist =
...
void plat_Accelerate(moveinfo_t * moveinfo)
{
	...
	// are we at full speed and need to start decelerating during this
	// move?
					   ...
					   csurface_t * surf)
{
	...
	if (ent->moveinfo.state == STATE_BOTTOM)
		...
		ent->nextthink = level.time + 1;	// the player is still on the
											// plat, so delay going down
}
...
void button_fire(edict_t * self)
{
	...
	if (self->activator) {
		if (chedit->value && CurrentIndex < MAXNODES
			&& !self->activator->deadflag
			&& self->activator == &g_edicts[1]) {
...
void SP_func_button(edict_t * ent)
{
	...
	dist =
		abs_movedir[0] * ent->size[0] + abs_movedir[1] * ent->size[1] +
...
//  VectorCopy(ent->s.origin,ent->monsterinfo.last_sighting);
	...
	if (1 /* !ent->health */ ) {
		...
		VectorSubtract(ent->moveinfo.start_origin,
					   ent->moveinfo.end_origin, abs_movedir);
...
void door_blocked(edict_t * self, edict_t * other)
{
	...
	for (i = 1; i <= maxclients->value; i++) {
		...
		if (ent->inuse && (ent->svflags & SVF_MONSTER) && ent->client) {
			if (ent->client->zc.waitin_obj == self
				&& ent->client->zc.zcstate) {
				...
				csurface_t * surf)
{
	...
	if (!(other->svflags & SVF_MONSTER)) {
		...
		gi.sound(other, CHAN_AUTO, gi.soundindex("misc/talk1.wav"), 1,
				 ATTN_NORM, 0);
	}
}
...
/////////
	if (fabs(ent->moveinfo.start_origin[2] - ent->moveinfo.end_origin[2]) >
		20) {
...
void SP_func_water(edict_t * self)
{
	...
	self->moveinfo.distance =
		abs_movedir[0] * self->size[0] + abs_movedir[1] * self->size[1] +
		abs_movedir[2] * self->size[2] - st.lip;
	VectorMA(self->pos1, self->moveinfo.distance, self->movedir,
			 self->pos2);
...
*/
void trigger_elevator_use(edict_t * self, edict_t * other,
						  edict_t * activator)
{
	edict_t *target;
...
void SP_func_timer(edict_t * self)
{
	...
	if (self->spawnflags & 1) {
		self->nextthink =
			level.time + 1.0 + st.pausetime + self->delay + self->wait +
			crandom() * self->random;
		self->activator = self;
	}
...
void func_conveyor_use(edict_t * self, edict_t * other,
					   edict_t * activator)
{
	if (self->spawnflags & 1) {
		self->speed = 0;
...
{
	if (!(other->svflags & SVF_MONSTER) && (!other->client)
		|| !Q_stricmp(other->classname, "bodyque")) {
		// give it a chance to go away on it's own terms (like gibs)
		T_Damage(other, self, self, vec3_origin, other->s.origin,
				 vec3_origin, 100000, 1, 0, MOD_CRUSH);
		// if it's still there, nuke it
...
void door_secret_die(edict_t * self, edict_t * inflictor,
...
void rotating_light_killed(edict_t * self, edict_t * inflictor,
						   edict_t * attacker, int damage, vec3_t point)
{
...
static void rotating_light_use(edict_t * self, edict_t * other,
							   edict_t * activator)
{
	if (self->spawnflags & START_OFF) {
