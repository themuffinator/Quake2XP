...
//skill 4
	FIRE_REFUGE | FIRE_EXPAVOID | FIRE_IGNORE | FIRE_QUADUSE,
//skill 5
	FIRE_EXPAVOID | FIRE_IGNORE | FIRE_QUADUSE | FIRE_AVOIDINV |
		FIRE_IGNORE | FIRE_JUMPROC,
//skill 6
	FIRE_EXPAVOID | FIRE_IGNORE | FIRE_QUADUSE | FIRE_AVOIDINV |
		FIRE_IGNORE | FIRE_JUMPROC,
//skill 7
...
//  if(zc->ctfstate == CTFS_SUPPORTER) zc->ctfstate = CTFS_OFFENCER;
	...
	for (i = 1; i <= maxclients->value && target == NULL; i++) {
		...
		if (!trent->inuse || ent == trent || zc->first_target == trent
			|| trent->deadflag)
			continue;
			...
			if (Bot_traceS(ent, trent)) {
				...
				if (!ctf->value && OnSameTeam(ent, trent)) {
					if (trent->client->zc.first_target) {
						if (Bot_traceS
							(ent, trent->client->zc.first_target))
							target = trent->client->zc.first_target;
					}
					if (trmin[2] < JumpMax && VectorLength(trmin) < 400) {
						yaw =
							(float) Bot[ent->client->zc.botindex].
							param[BOP_TEAMWORK];
						if (trent->client->pers.
...
//gi.bprintf(PRINT_HIGH,"Team stateON\n");
								if (trent->client->zc.tmplstate ==
									TMS_LEADER) {
									trent->client->zc.followmate = NULL;
									if (ent->client->zc.tmplstate ==
										TMS_LEADER) {
										if (random() < 0.5) {
											ent->client->zc.tmplstate =
												TMS_FOLLOWER;
											ent->client->zc.followmate =
												trent;
										} else {
											ent->client->zc.tmplstate =
												TMS_LEADER;
											ent->client->zc.followmate =
												NULL;
											trent->client->zc.tmplstate =
												TMS_FOLLOWER;
											trent->client->zc.followmate =
												ent;
										}
									} else {
										ent->client->zc.tmplstate =
					...
					if (trent->client->pers.inventory[k]) {
						...
						if (trent->client->zc.route_trace
							&& (trent->client->zc.routeindex - 2) >
							CurrentIndex) {
							zc->routeindex =
								trent->client->zc.routeindex - 2;
						}
...
//                      }
					...
					if (!tmpflg && target == NULL) {
						...
						if (pitch > 180)
							pitch = 360 - pitch;
						// if(ent->client->zc.zcstate & STS_WAITS) target
						// = trent;
						if (pitch <= vr) {
							yaw = Get_yaw(trmin);
							...
							if (yaw > 180)
								yaw = 360 - yaw;
							if (yaw <= hr
								|| (ent->client->zc.zcstate & STS_WAITS))
								target = trent;
						}
					}
					//
					if (!tmpflg && target == NULL && trent->mynoise
						&& trent->mynoise2) {
						if (Bot[ent->client->zc.botindex].
							param[BOP_NOISECHK]) {
							if (trent->mynoise->teleport_time >=
								(level.time - FRAMETIME)) {
								VectorSubtract(trent->mynoise->s.origin,
											   ent->s.origin, trmin);
								if (VectorLength(trmin) < 300) {
									pitch =
										(float) Bot[ent->client->zc.
													botindex].
										param[BOP_REACTION];
									if ((9 * random()) < pitch)
										target = trent;
				...
				if (Bot[ent->client->zc.botindex].param[BOP_NOISECHK]
					...
					if (trent->mynoise->teleport_time >=
						(level.time - FRAMETIME)) {
						AngleVectors(trent->client->v_angle, trmin, NULL,
									 NULL);
						VectorScale(trmin, 200, trmin);
						VectorAdd(trent->s.origin, trmin, trmin);
						rs_trace =
							gi.trace(trent->s.origin, NULL, NULL, trmin,
									 trent, MASK_SHOT);

						VectorSubtract(ent->s.origin, rs_trace.endpos,
									   trmin);
						if (VectorLength(trmin) < 500) {
							VectorCopy(rs_trace.endpos, trmin);
							rs_trace =
								gi.trace(ent->s.origin, NULL, NULL, trmin,
										 ent, MASK_SHOT);
							pitch =
								(float) Bot[ent->client->zc.botindex].
								param[BOP_REACTION];
...
void Bot_SearchItems(edict_t * ent)
{
	...
	if (ctf->value) {
		if (ent->moveinfo.state == SUPPORTER)
			...
			if (ent->moveinfo.state == CARRIER
				&& ent->target_ent->classname[6] == 'F')
				j = -1;
		} else if (ent->client->ctf_grapple != NULL)
			j = -1;
	}
	trent = zc->second_target;
	if (trent != NULL && !j) {
		x = random();
		if (trent->classname[0] == 'w')
			j = -1;
	...
	if (j == 0) {
		...
		// when not
			...
			if (ent->health > 70
				&& ent->client->pers.
				inventory[ITEM_INDEX(Fdi_ROCKETLAUNCHER
		...
		if ((level.framenum - ent->client->resp.enterframe) % 64 > 32)
			j = 0;
		else
			j = -1;
		...
		for (i = maxclients->value + 1; i < globals.num_edicts; i++) {
			...
			if (trent->solid != SOLID_TRIGGER) {
				if (ent->client->weaponstate == WEAPON_READY
					...
					if (entcln) {
						if (entcln[0] == 'f'	/* && trent->classname[5]
												   == 'b' */
							&& trent->health	// &&trent->moveinfo.state
												// == PSTATE_BOTTOM
							&& trent->takedamage)	// trent->moveinfo.wait
													// == 0)
						{
							if (trent->classname[5] == 'b'
								&& trent->monsterinfo.attack_finished >
								level.time)
								continue;

							trmax[0] =
								(trent->absmin[0] + trent->absmax[0]) / 2;
							trmax[1] =
								(trent->absmin[1] + trent->absmax[1]) / 2;
							trmax[2] =
								(trent->absmin[2] + trent->absmax[2]) / 2;
							rs_trace =
								gi.trace(ent->s.origin, NULL, NULL, trmax,
										 ent, MASK_SHOT);

							if (rs_trace.ent == trent) {
								// gi.bprintf(PRINT_HIGH,"kkkkkkk\n");
								// continue;
								VectorSubtract(trmax, ent->s.origin,
											   trmin);
...
//                              ent->client->ammo_index = 0;
			...
			if (trent->absmax[0] < touchmin[0])
				continue;
			if (touchmax[0] < trent->absmin[0])
				continue;
			if (trent->absmax[1] < touchmin[1])
				continue;
			if (touchmax[1] < trent->absmin[1])
				continue;
			if (trent->absmax[2] < touchmin[2])
				continue;
			if (touchmax[2] < trent->absmin[2])
				continue;

			VectorSubtract(trent->s.origin, ent->s.origin, trmin);
			yaw = VectorLength(trmin);

/*			if (trent->classname[0] == 'w' || trent->classname[0] == 'R')
				x = 0;
			else if (trent->classname[0]=='i') {
				if (trent->classname[5]=='q'
					|| trent->classname[5]=='f'
				...
				if (entcln[0] == 'i') {
					if (entcln[5] == 'h') {
						if (Q_stricmp(entcln, "item_health") == 0) {
							...
							if (fabs
								(trent->s.origin[2] - ent->s.origin[2] +
								 8) > JumpMax)
								continue;
							target = trent;
						...
						if (entcln[5] == 'q')	// item_quad
							...
							target = trent;
						else if (entcln[5] == 'p' && entcln[12] == 'h')
//							Q_stricmp(entcln, "item_power_shield") == 0)
							target = trent;
						else if (entcln[5] == 'p' && entcln[12] == 'c')	// Q_stricmp
																		// (entcln,
																		// "item_power_screen")
																		// ==
																		// 0)
					...
					if (entcln[5] == 'b') {	// ammo_bullets
						...
						if (ent->client->pers.
							inventory[ITEM_INDEX(Fdi_GRENADES)] /* FindItem("Grenades") */
						...
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_CELLS)] /* FindItem("Cells") */
							< ent->client->pers.max_cells)
							target = trent;
					} else if (entcln[5] == 'm') {	/* ammo_magslug */
						if (ent->client->pers.
							inventory[ITEM_INDEX(Fdi_MAGSLUGS)] /* FindItem("Mag Slug") */
							< ent->client->pers.max_magslug)
							target = trent;
					} else if (entcln[5] == 't') {	/* ammo_trap */
						if (ent->client->pers.inventory[ITEM_INDEX(Fdi_TRAP)] /* FindItem("Trap") */
							< ent->client->pers.max_trap)
							target = trent;
					}
				} else if (entcln[0] == 'w') {
					if (entcln[7] == 's' && entcln[8] == 'h') {	/* weapon_shotgun */
						if (!wstayf || (wstayf && !pickup_pri
										&& (!ent->client->pers.
											inventory[ITEM_INDEX(Fdi_SHOTGUN)] /* FindItem("Shotgun") */
											|| trent->spawnflags & (DROPPED_ITEM |
														  DROPPED_PLAYER_ITEM))))
							target = trent;
						...
						if (!wstayf || (wstayf
										&& (!ent->client->pers.
											inventory[ITEM_INDEX(Fdi_BOOMER)] /* FindItem("Ionripper") */
											|| trent->spawnflags & (DROPPED_ITEM |
														  DROPPED_PLAYER_ITEM))))
							target = trent;
						...
						if (!wstayf || (wstayf
										&& (!ent->client->pers.
											inventory[ITEM_INDEX(Fdi_BFG)] /* FindItem("BFG10K") */
											|| trent->spawnflags & (DROPPED_ITEM |
														  DROPPED_PLAYER_ITEM))))
							target = trent;
					}
					...
					if (entcln[2] == 'n') {
						...
						if (ent->client->zc.routeindex < CurrentIndex) {
							if (entcln[6] == 'X'
								&& Route[ent->client->zc.routeindex].
								state == GRS_ONTRAIN)
								target = trent;
							else if (entcln[6] == '2'
									 && Route[ent->client->zc.routeindex].
									 state == GRS_PUSHBUTTON
									 && Route[ent->client->zc.routeindex].
									 ent == trent)
								target = trent;
							else
			...
			if (target != NULL && !ctf->value) {
				if (!ent->waterlevel) {
					...
					if (x < -39) {
						yaw = iyaw/(-x);*/
//                      if (/*yaw < 0.5 &&*/ yaw > 2.5 /*&& iyaw > 64*/)
//							target = NULL;
						if (target != NULL) {
							if ((target->spawnflags & (DROPPED_ITEM | DROPPED_PLAYER_ITEM)) && x < -64)
								target = NULL;
						}
					}
			...
			if (target != NULL) {
				...
				if (conts & CONTENTS_LAVA)
					target = NULL;
				else if ((conts & CONTENTS_SLIME)
						 && ent->client->enviro_framenum <= level.framenum)
					target = NULL;
			}
			if (target != NULL && !ctf->value) {
				if ((target->s.origin[2] - ent->s.origin[2]) > 32 && !q) {
					x = target->moveinfo.start_origin[2] -
						ent->s.origin[2];
					if (x > 54 || x < -24)
						target = NULL;
					else {
						x = target->s.origin[2] -
							target->moveinfo.start_origin[2];
						if (x > 54 || x < 0)
							target = NULL;
					}
				} else if ((target->s.origin[2] - ent->s.origin[2]) <= 100
						   && q) {
					target = NULL;
				}
			}
			...
			if (target != NULL) {
				if (zc->second_target == NULL) {
					...
					if ((target->classname[0] == 'R'
						 && target->classname[0] != 'F')
						|| target->classname[0] == 'w'
						|| (target->classname[0] == 'i'
							&& target->classname[5] == 'q')
						|| (target->classname[0] == 'i'
							&& target->classname[5] == 't')
						|| (target->classname[0] == 'i'
							&& target->classname[5] == 'f')
						|| (target->classname[0] == 'i'
							&& target->classname[5] == 'i')) {
						VectorSubtract(ent->s.origin, target->s.origin,
									   trmin);
						if (ctf->value
							&& zc->second_target->classname[6] == 'F'
...
qboolean Bot_ExploAvoid(edict_t * ent, vec3_t v)
{
	...
	for (i = 0; i < MAX_EXPLINDEX; i++) {
		...
		if (ExplIndex[i] != NULL) {
			...
			absmax[0] += EXPLO_BOXSIZE;
			absmax[1] += EXPLO_BOXSIZE;
			absmax[2] += EXPLO_BOXSIZE;
			absmin[0] -= EXPLO_BOXSIZE;
			absmin[1] -= EXPLO_BOXSIZE;
			absmin[2] -= EXPLO_BOXSIZE;
...
qboolean CheckLaser(vec3_t pos, vec3_t maxs, vec3_t mins)
{
	...
	for (i = 0; i < MAX_LASERINDEX; i++) {
		...
		VectorSubtract(LaserIndex[i]->s.origin,
					   LaserIndex[i]->moveinfo.end_origin, v);
		L3 = VectorLength(v);

//		VectorCopy (LaserIndex[i]->s.origin, start);
		VectorMA(LaserIndex[i]->s.origin, L3 * L1 / (L1 + L2),
				 LaserIndex[i]->movedir, end);
			  ...
			  float *bottom)
{
	...
	tcontents = /* MASK_BOTSOLID */ MASK_BOTSOLIDX;	// MASK_PLAYERSOLID
													// /*|
													// CONTENTS_TRANSLUCENT*/;

//  if(!ent->waterlevel) tcontents |= CONTENTS_WATER;

	if (/* ent->client->zc.waterstate == WAS_FLOAT */ ent->waterlevel >= 1 /* 2 */ )
		tracelimit = 75;		// 75;//61;
...
//		trmax[2] = 32;
	...
	if (rs_trace.allsolid || rs_trace.startsolid || rs_trace.fraction != 1.0) {
		...
		for (i = 4; i < (tracelimit + 4); i += 4) {
			...
			rs_trace =
				gi.trace(trstart, trmin, vv /* trmax */ , trend, ent,
						 tcontents);
...
/*		trmax[2] = 32;
		...
		rs_trace =
			gi.trace(trend, trmin, trmax, trstart, ent,
					 tcontents | MASK_OPAQUE);

		*bottom = rs_trace.endpos[2] - ent->s.origin[2];

		if (0 /* rs_trace.fraction != 1.0 && rs_trace.plane.normal[2] < 0.7
...
//					gi.bprintf(PRINT_HIGH,"apooX\n");
		...
		if (0 /* !ent->client->zc.route_trace */ ) {
			if (rs_trace.plane.normal[2] > 0
				&& rs_trace.plane.normal[2] < 0.7)
				*bottom /= rs_trace.plane.normal[2];
		}
		...
		if (!ent->waterlevel
			&& (ent->client->ps.pmove.pm_flags & PMF_DUCKED)
			&& ent->groundentity == NULL && ent->velocity[2] > 10
			&& trmax[2] == 4)
			return 2;
...
int Bot_Watermove(edict_t * ent, vec3_t pos, float dist, float upd)
{
	...
	rs_trace = gi.trace(ent->s.origin, ent->mins, ent->maxs, trmax, ent,
						/* MASK_BOTSOLID */ MASK_BOTSOLIDX);
...
//		return false;
	...
	for (i = 0; i < 360; i += 10) {
		...
		rs_trace =
			gi.trace(trmax /* ent->s.origin */ , ent->mins, ent->maxs,
					 trmin, ent, MASK_BOTSOLIDX);

//      yaw = VectorLength(trmax);
		if (!rs_trace.allsolid && !rs_trace.startsolid) {
			VectorAdd(rs_trace.endpos, touchmin, trmax);
			rs_trace =
				gi.trace(trmax, ent->mins, ent->maxs, trmax, ent,
						 MASK_BOTSOLIDX);
...
void Set_Combatstate(edict_t * ent, int foundedenemy)
{
	...
	if (client->zc.zcstate & STS_LADDERUP)
		return;
	...
	if (!target->inuse || target->deadflag || target->solid != SOLID_BBOX) {
		...
		if ((9 * random()) <
			Bot[client->zc.botindex].param[BOP_COMBATSKILL])
...
//      return;
	...
	if (!(client->zc.zccmbstt & CTS_ENEM_NSEE))
		...
		Combat_LevelX(ent, foundedenemy, enewep, aim, distance, combskill);
		...
		if (Bot_moveT(ent, yaw, temppos, tdist, &bottom) == true) {
			...
			if (bottom <= JumpMax && bottom > -JumpMax) {
				if (Get_FlyingSpeed(bottom, x, dist, &speed)) {
					...
					Set_BotAnim(ent, ANIM_JUMP, FRAME_jump1 - 1,
								FRAME_jump6);
...
//          zc->route_trace = false;
		...
		if (Route[zc->routeindex].state == GRS_ONTRAIN) {
			if (1 /* Route[zc->routeindex].ent->trainteam == NULL */ ) {
				vv[0] +=
					FRAMETIME * Route[zc->routeindex].ent->velocity[0] * x;
				vv[1] +=
					FRAMETIME * Route[zc->routeindex].ent->velocity[1] * x;
			}
		}
...
//                  }
	...
	if (mode == 2) {
		// vel�l���̗���
		if (Route[zc->routeindex].state == GRS_ONTRAIN) {
			if (1				/* Route[zc->routeindex].ent->trainteam ==
								   NULL */ ) {
//				gi.bprintf(PRINT_HIGH,"Go!\n"); //ppx
	...
	if (l < MOVE_SPD_RUN && mf == true) {
		...
		ent->velocity[2] += VEL_BOT_JUMP;
		gi.sound(ent, CHAN_VOICE, gi.soundindex("*jump1.wav"), 1,
				 ATTN_NORM, 0);
		PlayerNoise(ent, ent->s.origin, PNOISE_SELF);	// pon
...
void Get_RouteOrigin(int index, vec3_t pos)
{
	...
	if (Route[index].state <= GRS_ITEMS
		|| Route[index].state >= GRS_GRAPSHOT) {
		if (Route[index].state == GRS_ITEMS) {
	...
	// when train
		...
		if (Route[index].ent->target_ent) {
			if (VectorCompare
				(Route[index].Tcourner,
				 Route[index].ent->target_ent->s.origin)) {
				VectorCopy(Route[index].ent->union_ent->s.origin, pos);
				pos[2] += 8;
				return;
			}
		...
		if (Route[index].ent->union_ent) {
			...
			pos[0] =
				(Route[index].ent->absmin[0] +
				 Route[index].ent->absmax[0]) / 2;
			pos[1] =
				(Route[index].ent->absmin[1] +
				 Route[index].ent->absmax[1]) / 2;
...
void Search_NearlyPod(edict_t * ent)
{
	...
	if (Route[ent->client->zc.routeindex].state >= GRS_ITEMS)
		return;
...
int Get_KindWeapon(gitem_t * it)
{
	...
	else if (it->weaponthink == Weapon_Trap)
		return WEAP_TRAP;
	...
	else if (it->weaponthink == Weapon_Ionripper)
		return WEAP_BOOMER;
	else if (it->weaponthink == Weapon_Railgun)
		return WEAP_RAILGUN;
	else if (it->weaponthink == Weapon_Phalanx)
		return WEAP_PHALANX;
...
void Bots_Move_NORM(edict_t * ent)
{
	...
	if (i & CONTENTS_SOLID)
		T_Damage(ent, ent, ent, ent->s.origin, ent->s.origin,
				 ent->s.origin, 100, 1, 0, MOD_CRUSH);
	...
	if (VectorCompare(ent->s.origin, ent->s.old_origin)) {
		if (ent->groundentity == NULL && !ent->waterlevel) {
			...
			v[2] -= 1.0;
			rs_trace =
				gi.trace(ent->s.origin, ent->mins, ent->maxs, v, ent,
						 MASK_BOTSOLIDX);
			if (!rs_trace.allsolid && !rs_trace.startsolid)
...
//gi.bprintf(PRINT_HIGH,"Target Flag On\n");
						...
						break;
							   ...
							   && Route[i].ent->solid == SOLID_TRIGGER) {
						// Quad
			...
			if (j) {
				...
				if (zc->targetindex < zc->routeindex
					|| zc->targetindex >= CurrentIndex)
					zc->targetindex = zc->routeindex;
				for (i = zc->targetindex + 1; i < (zc->targetindex + 25);
					 i++) {
					if (i > CurrentIndex)
						break;
					if (Route[i].state == GRS_ITEMS) {
	...
	// ducking check
	if (ent->client->ps.pmove.pm_flags & PMF_DUCKED) {
		if (ent->client->zc.battleduckcnt > 0 && ent->groundentity)
			goto DCHCANC;
...
//      else gi.bprintf(PRINT_HIGH,"failed %i %i\n",rs_trace.startsolid ,rs_trace.allsolid);
		...
		rs_trace =
			gi.trace(ent->s.origin, ent->mins, v, ent->s.origin, ent,
					 MASK_BOTSOLIDX);
  ...
  DCHCANC:
	...
	if (ent->client->ps.pmove.pm_flags & PMF_DUCKED && ent->groundentity)
		...
		if (!ent->waterlevel) {
			if (chedit->value || !Bot[zc->botindex].param[BOP_WALK]
				|| !ent->groundentity)
...
//  else
	...
	if (zc->second_target != NULL && !(zc->zcstate & STS_WAITSMASK)) {
		if (zc->second_target->solid != SOLID_TRIGGER
			|| !zc->second_target->inuse) {
			zc->second_target = NULL;
		} else if (!Bot_trace(ent, zc->second_target)) {
			zc->second_target = NULL;
		} else if ((zc->second_target->s.origin[2] - ent->s.origin[2]) > 32
				   && zc->waterstate != WAS_IN) {
			...
			if (temppos[2] > 32) {
				...
				if (temppos[0] <= (ent->absmax[0] + 32)
					&& temppos[0] >= (ent->absmin[0] + 32))
					if (temppos[1] <= (ent->absmax[1] + 32)
...
//          }
			if (!zc->first_target
				&& ent->client->pers.weapon != Fdi_GRAPPLE) {
				for (i = 0; i < (5 * 2); i++) {
					if ((zc->routeindex + i) >= CurrentIndex)
						break;
...
//                      VectorCopy(Route[zc->routeindex - 1].Pt,ent->s.origin);
						VectorSubtract(Route[zc->routeindex - 1].Tcourner,
									   vv, v);
...
//                      }
						...
						if (zc->first_target == NULL && ent->groundentity)
							trace_priority = TRP_ALLKEEP;
						zc->routeindex--;	/* trace_priority =
											   TRP_ALLKEEP; */
					}
				}
				...
				if (zc->nextcheck < (level.time + FRAMETIME * 10)) {
					VectorSubtract(zc->pold_origin, ent->s.origin,
								   temppos);
				...
				if (ent->client->ctf_grapplestate ==
					...
					if (Route[zc->routeindex].state != GRS_GRAPRELEASE) {
						for (i = 0; (zc->routeindex - i) > 0; i++) {
							if (Route[zc->routeindex - i].state ==
								GRS_GRAPSHOT)
								break;
						}
						if ((zc->routeindex - i) > 0) {
							for (j = 0;
								 (zc->routeindex - i + j) < CurrentIndex;
								 j++) {
								if (Route[zc->routeindex - i + j].state ==
									GRS_GRAPRELEASE)
									break;
							}
...
//                      ShowGun(ent);
				...
				} else if (ent->client->ctf_grapplestate ==
			...
			if (Route[zc->routeindex].state == GRS_GRAPRELEASE
				...
				if (ent->client->ctf_grapplestate == CTF_GRAPPLE_STATE_FLY)
					...
					if (1) {
						...
						yaw = VectorLength(v);
						if (yaw <= (Route[zc->routeindex].Tcourner[0]	/* +
																		   32
																		 */ )) {
...
//gi.bprintf(PRINT_HIGH,"Groff 0!\n");
				...
				if (k) {
					if (zc->routeindex < CurrentIndex) {
						if (Route[zc->routeindex].state == GRS_GRAPSHOT) {
							if (1
								/* TraceX(ent,Route[zc->routeindex +
								   1].Tcourner) */
								)
								zc->routeindex++;
						}
...
//      if(trace_priority == TRP_ALLKEEP) goto VCHCANSEL;
		...
		if (Route[zc->routeindex].state >= GRS_NORMAL)
			Search_NearlyPod(ent);
...
//              gi.bprintf(PRINT_HIGH,"shit1");
			...
			if (Route[zc->routeindex].state > GRS_ITEMS
				...
				if (0
					/* Route[zc->routeindex].state == GRS_ONPLAT &&
					   Route[zc->routeindex].ent->moveinfo.state !=
					   PSTATE_BOTTOM */
					) {
#ifdef _DEBUG
					...
					zc->rt_releasetime =
						level.time + FRAMETIME * POD_RELEFRAME;
						   ...
						   /* Route[zc->routeindex].ent->union_ent->solid
						      != SOLID_TRIGGER */
					) {
#ifdef _DEBUG
	...
	if (trace_priority == TRP_ALLKEEP) {
		if (ent->client->ctf_grapple) {
			rs_trace =
				gi.trace(ent->s.origin, ent->maxs, ent->mins,
						 ent->s.origin, ent, MASK_BOTSOLIDX);
			if (rs_trace.allsolid || rs_trace.startsolid
				|| rs_trace.fraction != 1.0)
				ent->client->ps.pmove.pm_flags |= PMF_DUCKED;
		}
	...
	if (!chedit->value && zc->second_target == NULL) {
			...
			if (zc->secondinterval > 40) {
				if (!(zc->zcstate & STS_WAITSMASK)) {
					...
					if (zc->second_target != NULL && pickup_pri) {
						for (i = zc->routeindex; i < (zc->routeindex + 20);
							 i++) {
							if (i >= CurrentIndex)
								break;
							if (Route[i].state == GRS_ITEMS) {
		...
		if (zc->secondinterval >
			40 /* zc->second_target != NULL *//* && !k */ ) {
		...
		if (ent->client->zc.objshot)
			goto VCHCANSEL;		// object shot!!
	}
	// --------------------------------------------------------------------------------------
	// go up ladder
	// --------------------------------------------------------------------------------------
...
//          if(VectorCompare(ent->s.origin,ent->s.old_origin)) ent->velocity[2] += 50;
		...
		rs_trace =
			gi.trace(ent->s.origin, trmin /* ent->mins */ , ent->maxs,
					 touchmax, ent, MASK_BOTSOLID /* MASK_PLAYERSOLID */ );
		...
		if (!(rs_trace.contents & CONTENTS_LADDER)
			...
			gi.bprintf(PRINT_HIGH,
					   "ladder OFF1!!\ncont %x %x\nall %i\nstart %i\n",
					   rs_trace.contents, i, rs_trace.allsolid,
					   rs_trace.startsolid);
#endif
			if (ent->velocity[2] <= VEL_BOT_LADRUP && !ent->waterlevel)
				ent->velocity[2] = VEL_BOT_LADRUP;
			zc->zcstate &= ~STS_LADDERUP;
...
//          ent->moveinfo.speed = 1.0;
			...
			if (x == 0	/* VectorCompare(ent->s.origin,ent->s.old_origin) */ ) {
				...
				trmax[2] += 64;
				rs_trace =
					gi.trace(trmin, NULL, NULL, trmax, ent, MASK_BOTSOLID);
				...
				if (x != 0.0) {
					...
					VectorAdd(ent->s.origin, touchmin, trmin);
					rs_trace =
						gi.trace(ent->s.origin, ent->mins, ent->maxs,
								 trmin, ent, MASK_BOTSOLID);
					if (rs_trace.startsolid || rs_trace.allsolid)
						x = 0;
					else
						VectorCopy(rs_trace.endpos, ent->s.origin);
	...
	if (zc->second_target != NULL) {
		if ((zc->second_target->s.origin[2] - ent->s.origin[2]) > 32
			&& !ent->waterlevel) {
			x = zc->second_target->moveinfo.start_origin[2] -
				ent->s.origin[2];
			if (x <= 32 && x > -24
				...
				if (!k) {
					if (!ent->groundentity && !ent->waterlevel) {
						...
						if (trace_priority < TRP_MOVEKEEP)
							zc->moveyaw = Get_yaw(temppos);
						if (trace_priority < TRP_ANGLEKEEP)
							ent->s.angles[YAW] = zc->moveyaw;
					}

					if (x < dist && fabs(temppos[2]) < 24)
						dist = x;	// pon
				}
				j = -1;
			}
			...
			if (!ent->groundentity && !ent->waterlevel
				...
				if (!k) {
					if (trace_priority < TRP_MOVEKEEP)
						zc->moveyaw = Get_yaw(temppos);	// set the movin'
														// yaw
					if (trace_priority < TRP_ANGLEKEEP)
						ent->s.angles[YAW] = zc->moveyaw;
					if (x < dist && fabs(yaw) < JumpMax)
						dist = x;
				}
			}
			j = -1;
...
/*	if (zc->first_target != NULL) {
	...
	if (ctf->value
		...
		if (ctf->value) {
			if (zc->ctfstate == CTFS_SUPPORTER) {
				if (zc->followmate) {
					if (zc->followmate->inuse)
						if (zc->followmate->client->zc.ctfstate !=
							CTFS_CARRIER) {
			...
			if (zc->tmplstate == TMS_FOLLOWER && zc->second_target == NULL)
				j = 1;
			else
				j = 0;
		}
		...
		if (j /* zc->tmplstate == TMS_FOLLOWER && zc->second_target == NULL */ ) {
			if (zc->followmate) {
				...
				if (!zc->followmate->inuse || zc->followmate->deadflag
					|| zc->matelock <= level.time) {
					if (ctf->value)
						zc->ctfstate = CTFS_OFFENCER;
					else
					...
					VectorSubtract(zc->followmate->s.origin, ent->s.origin,
								   v);
					if (VectorLength(v) < 200) {
						if (k && zc->followmate->client->zc.route_trace
							&& (zc->followmate->client->zc.routeindex -
							...
							zc->moveyaw = Get_yaw(v);
							// if(VectorLength(v) < 100) trace_priority =
							// TRP_ALLKEEP;
						...
						if (zc->followmate->client->zc.route_trace
...
//gi.bprintf(PRINT_HIGH,"Wrong way 1\n");
						for (i = 0; i < (MAXLINKPOD - 1); i++) {
							if (!Route[zc->routeindex].linkpod[i])
								break;
							k = Route[Route[zc->routeindex].linkpod[i]].
								linkpod[MAXLINKPOD - 1];
							if (!(k & CTF_FLAG2_FLAG)) {
		...
		if (!zc->route_trace && zc->rt_releasetime <= level.time) {
			...
			for (i = 0; i < CurrentIndex && i < MAX_SEARCH; i++) {
				if (Route[zc->routeindex].state == GRS_GRAPHOOK) {
					...
					continue;
				} else if (Route[zc->routeindex].state == GRS_GRAPRELEASE) {
					++zc->routeindex;
					continue;
						...
						if ((!j || (v[2] - ent->s.origin[2]) > JumpMax)
							...
							if (x == 0 /* < dist */  || k) {
								if (it_ent->nextthink >= level.time)
									zc->rt_locktime =
										level.time +
										FRAMETIME * POD_LOCKFRAME;
								goto VCHCANSEL;	// if center position move
												// cancel
							}
							if (x < dist)
								dist = x;
...
/*				if(Route[zc->routeindex].state == GRS_ITEMS)
				...
				if (Route[zc->routeindex].state == GRS_PUSHBUTTON) {
					...
					if (it_ent->health
						&& (it_ent->takedamage
							|| it_ent->moveinfo.state != PSTATE_TOP)) {
						k = 2;
					...
					rs_trace =
						gi.trace(ent->s.origin, touchmin, touchmax, v, ent,
								 MASK_SHOT);
					if (rs_trace.fraction != 1.0 && rs_trace.ent) {
						if ((rs_trace.ent->health
							 || rs_trace.ent->takedamage)
...
//gi.bprintf(PRINT_HIGH,"ooooooo!\n");
					...
					if (k == 2) {
						...
						rs_trace =
							gi.trace(it_ent->union_ent->s.origin, NULL,
									 NULL, trmin, it_ent->union_ent,
									 MASK_SHOT);
						VectorSubtract(rs_trace.endpos, ent->s.origin,
									   trmax);
					} else
...
//if(ent->client->newweapon) gi.bprintf(PRINT_HIGH,"selected %s\n",ent->client->newweapon->pickup_name);
				...
				if (Route[zc->routeindex].state == GRS_ONTRAIN && !zc->waterstate	/* <
																					   WAS_IN
																					 */
					/* ent->groundentity */ ) {
...
/*					if(!Q_stricmp(ent->groundentity->classname, "func_train"))
				...
				if (ent->client->ps.pmove.pm_flags & PMF_DUCKED)
					...
					if (zc->waterstate == WAS_IN)
						f1 = 24;
					else if ( /* zc->waterstate == WAS_FLOAT */ ent->
							 waterlevel && ent->waterlevel < 3) {
						if (v[0] == ent->s.origin[0]
							&& v[1] == ent->s.origin[1])
							f1 = -300;
						else
							f1 = -(JumpMax + 64);
				...
				if (Route[zc->routeindex].state == GRS_ONROTATE)
					yaw = -48;
				else
					yaw = 12;
				if (v[0] <= (ent->absmax[0] - yaw)
					&& v[0] >= (ent->absmin[0] + yaw)) {
					if (v[1] <= (ent->absmax[1] - yaw)
						&& v[1] >= (ent->absmin[1] + yaw)) {
						if ((v[2] <= (ent->absmax[2] - f1)
							 && v[2] >= (ent->absmin[2] + f2))
...
//                          if(yaw > JumpMax) k = true;
							...
							if (!k) {
								...
								if (x < dist && fabs(yaw) < 20	/* JumpMax
									...
									i = Bot_moveT(ent, iyaw, temppos, x,
												  &bottom);
									...
									if (Route[zc->routeindex].state ==
										...
										if (j) {
											...
											VectorCopy(v, trmin);
											dist -= x;
											if (Route[zc->routeindex].
												state <= GRS_ITEMS) {
												if (zc->havetarget) {
													for (i = 0; i < (MAXLINKPOD - (ctf->value != 0)); i++) {
														j = Route[zc->routeindex].
															linkpod[i];
														if (!j)
															break;
...
//gi.bprintf(PRINT_HIGH,"change index2\n");
										...
										if ((Route[zc->routeindex].state == GRS_ITEMS || Route[zc->routeindex]. state == GRS_NORMAL)
							...
							if ((zc->routeindex - 1) >= 0 &&
								(Route[zc->routeindex].state == GRS_ONPLAT
								 ...
								 GRS_ONTRAIN)) {
								Get_RouteOrigin(zc->routeindex - 1, v);
								if (fabs(v[2] - ent->s.origin[2]) <=
									JumpMax) {
									if (zc->waterstate < WAS_IN
										/* &&
										   Route[zc->routeindex].ent->trainteam
										   == NULL */
										&& Route[zc->routeindex].ent->
...
gi.bprintf(PRINT_HIGH,"aw shit!!\n");*/
				if (it_ent->pos1[2] > it_ent->pos2[2]
					&&
					((it_ent->moveinfo.state == PSTATE_UP
					  && it_ent->velocity[2] > 0)
					 || it_ent->moveinfo.state == PSTATE_BOTTOM)
...
//gi.bprintf(PRINT_HIGH,"YEAH onplat!!\n");
						...
						/* for(i = 0;i < 10;i++) { if((zc->routeindex + i)
						   >= CurrentIndex) break;
						   if(!Route[zc->routeindex + i].index) break;
						   if(Route[zc->routeindex + i].state ==
						   GRS_ONPLAT && Route[zc->routeindex + i].ent ==
						   zc->waitin_obj) { zc->routeindex += i + 1;
						   break; } } */
					}
				}
			}
						...
						if ((k + i) < CurrentIndex) {
							if (Route[k + i].state == GRS_ONTRAIN) {
								if (Route[k + i].ent == it_ent)
									...
									if (VectorCompare(Route[k + i].Tcourner,	/* e
																				 */
													  it_ent->target_ent->
													  s.origin)) {
										j = true;
...
//gi.bprintf(PRINT_HIGH,"On train2!!\n");
						...
						if ((it_ent->s.origin[2] -
							 it_ent->s.old_origin[2]) > -2
							&& trace_priority
							&& zc->second_target == NULL) {
//gi.bprintf(PRINT_HIGH,"On train3!!\n");
	...
	// on plat
	else if (((zc->zcstate & STS_W_ONPLAT)
			  || (zc->zcstate & STS_W_COMEPLAT)
...
//              if((v[2] - ent->s.origin[2]) > JumpMax)
		...
		if ( /* j || */ k != true) {
			...
			if (zc->zcstate & STS_W_COMEPLAT) {
				...
				trmax[0] =
					(zc->waitin_obj->absmin[0] +
					 zc->waitin_obj->absmax[0]) / 2;
				trmax[1] =
					(zc->waitin_obj->absmin[1] +
					 zc->waitin_obj->absmax[1]) / 2;
				trmax[2] = 0;
			}
	...
	// on train
		...
		if (zc->route_trace) {
			...
			if (TraceX(ent, v)) {
				x = v[2] -
					/* zc->waitin_obj->union_ent->s.origin[2]; */
					ent->s.origin[2];
				if (x <= JumpMax) {
...
//gi.bprintf(PRINT_HIGH,"ponko1!\n");
				...
				// trmax[0] = (zc->waitin_obj->absmin[0] +
				// zc->waitin_obj->absmax[0]) / 2;
				// trmax[1] = (zc->waitin_obj->absmin[1] +
				// zc->waitin_obj->absmax[1]) / 2;
				trmax[2] += 8;
				VectorSubtract(trmax, ent->s.origin, temppos);
				yaw = temppos[2];
...
//                  goto VCHCANSEL; //if center position move cancel
	...
	// wait for door open
		...
		if (!trace_priority || zc->waitin_obj->moveinfo.state == PSTATE_TOP
			/* || (zc->waitin_obj->moveinfo.state == PSTATE_DOWN) */ ) {
				...
				if (!zc->waitin_obj->union_ent) {
					...
					trmin[1] =
						(zc->waitin_obj->absmin[1] +
						 zc->waitin_obj->absmax[1]) / 2;
					trmin[2] =
						(zc->waitin_obj->absmin[2] +
						 zc->waitin_obj->absmax[2]) / 2;
				} else
					VectorCopy(zc->waitin_obj->union_ent->s.origin, trmin);
				trmin[2] += 8;
...
*/
	...
	// rocket jump
	// ent->client->weaponstate = WEAPON_READY;
	if (ent->groundentity && ent->client->weaponstate == WEAPON_READY
...
//              it->use(ent,it);
				...
				zc->second_target = NULL;
		} else if ((zc->second_target->s.origin[2] - ent->s.origin[2]) >
				   100) {
			if (ent->health <= 70)
				zc->second_target = NULL;
		}
  ...
  GOMOVE:
	...
	if (!ent->groundentity && !ent->waterlevel && !ent->client->zc.trapped) {
		if (ent->velocity[2] > VEL_BOT_JUMP
		...
		for (x = 0; x < 90; x += 10) {
			...
			if (i)				// true || (i == 2 && ent->velocity > 0))
			{
				...
				if (!ent->waterlevel
					...
					&& zc->route_trace && !(zc->zcstate & STS_LADDERUP)
					&& !(zc->zcstate & STS_SJMASK)
					&& (zc->routeindex + 1) < CurrentIndex
					&& ent->velocity[2] >= 100
					&& ent->velocity[2] <
					(100 + ent->gravity * sv_gravity->value * FRAMETIME)) {
					Get_RouteOrigin(zc->routeindex, v);
					...
					if ((vv[2] - v[2]) > JumpMax)
						...
						k = 2;
					else if (!TargetJump_Chk(ent, vv, 0)
							 && VectorLength(trmin) < 64) {
...
//gi.bprintf(PRINT_HIGH,"jump fail!\n");
	...
	if (ent->groundentity || ent->waterlevel) {
		...
		if (zc->waterstate)
			f1 = BOTTOM_LIMIT_WATER;
		else
			f1 = -JumpMax;		// BOTTOM_LIMIT;//dropable height
		...
		for (x = 0; x <= 180 && dist != 0; x += 10) {
			...
			if (j = Bot_moveT(ent, yaw, temppos, dist, &bottom)) {
				...
				if (x == 0 && /* bottom < 20 && */ !ent->waterlevel
					&& !(ent->client->ps.pmove.pm_flags & PMF_DUCKED)) {
					if (zc->second_target) {
						if (((zc->second_target->s.origin[2] + 8) -
							 (ent->s.origin[2] + bottom)) > f2) {
...
//                      FRAMETIME * (ent->velocity[2] - ent->gravity * sv_gravity->value * FRAMETIME)
						if ((v[2] - (ent->s.origin[2] + bottom)) > f2 ||
							...
							if ((v[2] - ent->s.origin[2]) <= JumpMax) {
								if (Route[zc->routeindex].state ==
									GRS_ONTRAIN && zc->waterstate < WAS_IN)
									break;
								if (zc->routeindex > 0)
									if (Route[zc->routeindex - 1].state ==
										GRS_ONTRAIN
										&& Route[zc->routeindex - 1].ent ==
										ent->groundentity)
										break;
							}
						} else if (ent->groundentity) {
...
//gi.bprintf(PRINT_HIGH,"%s!\n",ent->groundentity->classname);
							...
							} else if (Route[zc->routeindex].state ==
									   GRS_ONROTATE) {
								if (!TraceX(ent, v)
									|| !HazardCheck(ent, v))
									break;
				...
				// jumpable1
				if (			/* ((bottom > 20 && !ent->waterlevel) ||
								   (bottom > 0 && (ent->waterlevel == 2 ||
								   (ent->waterlevel == 1 &&
					...
					if (k == 1 /* !ent->waterlevel */ ) {
						...
						gi.sound(ent, CHAN_VOICE,
								 gi.soundindex("*jump1.wav"), 1, ATTN_NORM,
								 0);
						PlayerNoise(ent, ent->s.origin, PNOISE_SELF);	// pon
					} else {
						...
						if (ent->velocity[2] < VEL_BOT_WJUMP /*=1*/
							|| VectorCompare(ent->s.origin,
											 ent->s.old_origin)) {
							ent->velocity[2] /* + */  = VEL_BOT_WJUMP;	// (/*VEL_BOT_WJUMP*/
																		// 110
																		// /*+
																		// bottom*/);
				...
				// dropable1
				else if (bottom <= f3 && (bottom >= f1 ||	/* zc->waterstate
															 */
										  ent->waterlevel /* 2 */ )) {
//                  ent->client->anim_priority = ANIM_BASIC;
					if (bottom < 0 && !zc->waterstate
						/* (ent->waterlevel &&
						   !zc->waterstate/*ent->waterlevel < 2) */
						) {
						f2 = FRAMETIME * (ent->velocity[2] -
										  ent->gravity *
										  sv_gravity->value * FRAMETIME);
						if (bottom >= f2 && ent->velocity[2] < 0 /* 20 */ )
							temppos[2] += bottom;
						else
							temppos[2] += f2;	// 20;
					}
...
//gi.bprintf(PRINT_HIGH,"ducked1!!\n");
				...
				// dropable?1
				else if (bottom < f1 && !zc->waterstate	/* !ent->waterlevel
														 */
						 && x <= 30) {
					if (ladderdrop && zc->ground_contents & CONTENTS_LADDER
						&& bottom != -9999) {
						VectorCopy(temppos, ent->s.origin);
						zc->moveyaw = yaw;
...
//else if(ent->client->zc.waterstate == 1 && x == 0) gi.bprintf(PRINT_HIGH,"maaaap %i\n",j);
			...
			if (x == 0 && (zc->battlemode & FIRE_SHIFT))
...
//                  if(VectorLength(vv) < 13) VectorCopy(v,ent->s.origin);
			...
			if ((!ent->groundentity && !zc->waterstate && k
				 && ent->velocity[2] < 1)
				|| (zc->waterstate == WAS_IN
					&& (ent->client->ps.pmove.pm_flags & PMF_DUCKED))) {
				if (Bot_Watermove(ent, temppos, dist, -7 /* 8 */ )
					&& k != 3) {
					VectorCopy(temppos, ent->s.origin);
				}
			}
...
*/
	...
	if (!zc->route_trace && trace_priority && !zc->second_target
		&& random() < 0.2) {
		VectorCopy(ent->s.origin, v);
		VectorCopy(ent->mins, touchmin);
		touchmin[2] += 16;
		...
		VectorAdd(v, trmin, trmax);
		rs_trace =
			gi.trace(v, NULL /* touchmin */ , NULL /* touchmax */ , trmax,
					 ent, MASK_BOTSOLIDX /* MASK_PLAYERSOLID */ );

		if (x > rs_trace.fraction && x > 0.5)
			zc->moveyaw = f1;
	}
	// --------------------------------------------------------------------------------------
	// push button
			...
			if (trent->classname) {
				if (!Q_stricmp(trent->classname, "func_button")) {
					...
					break;
						   ...
						   || !Q_stricmp(trent->classname,
										 "func_door_rotating")) {
					if (trent->targetname == NULL && !trent->takedamage
						&& ent->groundentity != trent) {
						k = 2;
						it_ent = trent;
						break;
					}
...
//          if(!k) buttonuse = true;//it_ent->use(it_ent,ent,it_ent/*ent*/);
			if (!k && it_ent->target) {
				...
				for (i = maxclients->value + 1; i < globals.num_edicts;
					 i++, e++) {
...
//gi.bprintf(PRINT_HIGH,"yea4  %i %s\n",e->moveinfo.state,e->classname);
						if (e->classname[0] == 't') {
							if (!Q_stricmp(e->classname, "trigger_relay")) {
								...
								if (!Q_stricmp
									(e->classname, "target_laser")
									|| !Q_stricmp(e->classname,
												  "target_mal_laser")) {
								if (e->spawnflags & 1) {
									...
									// e->moveinfo.end_origin[2])) > 54 )
							{
								...
								if (!zc->route_trace	/* || zc->routeindex <= 0 */
									...
									v[2] =
										(it_ent->absmin[2] +
										 it_ent->absmax[2]) / 2;
									VectorSubtract(it_ent->union_ent->s.
												   origin, v, temppos);
									VectorScale(temppos, 3, v);
									VectorAdd(ent->s.origin, v,
											  zc->movtarget_pt);
								} else {
									/* Get_RouteOrigin(zc->routeindex -
									   1,v);
...
//if(zc->zcstate & STS_WAITSMASK ) gi.bprintf(PRINT_HIGH,"Door Use\n");
		...
		if (it_ent->moveinfo.state == PSTATE_BOTTOM) {
			...
			if (it_ent->flags & FL_TEAMSLAVE) {
				trmin[0] =
					(it_ent->teammaster->absmin[0] +
					 it_ent->teammaster->absmax[0]) / 2;
				trmin[1] =
					(it_ent->teammaster->absmin[1] +
					 it_ent->teammaster->absmax[1]) / 2;
  ...
  VCHCANSEL:
	...
	if (k && trace_priority && !zc->second_target
		...
		VectorAdd(ent->s.origin, touchmin, touchmax);
		rs_trace =
			gi.trace(ent->s.origin, trmin, ent->maxs, touchmax, ent,
					 MASK_BOTSOLID);
		front = rs_trace.ent;
		...
		if (!tempflag && !zc->waterstate) {
			trmax[2] += 32;
			rs_trace =
				gi.trace(ent->s.origin, trmin, trmax, touchmax, ent,
						 MASK_BOTSOLID);
			if (rs_trace.contents & CONTENTS_LADDER)
				tempflag = 2;
		}
		if (!tempflag && ent->groundentity) {
			...
			rs_trace =
				gi.trace(ent->s.origin, trmin, ent->maxs, v, ent,
						 MASK_BOTSOLID);
			if (rs_trace.contents & CONTENTS_LADDER)
				tempflag = 3;
		}
		...
		if (tempflag) {
			...
			touchmax[2] += 8190;
			rs_trace =
				gi.trace( /* temppos */ trmax, trmin, ent->maxs, touchmax,
						 ent, MASK_SOLID);
...
//if((rs_trace.contents & CONTENTS_LADDER)) gi.bprintf(PRINT_HIGH,"damn!\n");
			...
			VectorAdd(rs_trace.endpos, touchmin, touchmax);
			rs_trace =
				gi.trace(temppos, trmin, ent->maxs, touchmax, ent,
						 MASK_BOTSOLID);
...
//              goto VCHCANSEL;
  ...
  VCHCANSEL_L:
	...
	// not ducked
		...
		if (ent->client->zc.n_duckedtime < FRAMETIME * 10)
			ent->client->zc.n_duckedtime += FRAMETIME;
		ent->maxs[2] = 32;
		ent->viewheight = 22;
	}
...
/*		if(ent->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY)
		...
		if (ent->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY) {
			...
			if (ent->groundentity && ent->velocity[2] < 0)
				...
				ent->velocity[2] += JumpMax;	// ent->gravity *
												// sv_gravity->value *
												// FRAMETIME * 4;
				...
				ent->velocity[2] +=
					ent->gravity * sv_gravity->value * FRAMETIME * 2;
		}
	} else {
		if (ent->waterlevel > 2) {
			ent->velocity[0] = 0;
			ent->velocity[1] = 0;	/* VectorClear(ent->velocity); */
		} else if (ent->waterlevel && !ent->groundentity
				   && ent->velocity[2] < 0)
			VectorClear(ent->velocity);	// ent->velocity[2] = 0;
	}
//ZOID
