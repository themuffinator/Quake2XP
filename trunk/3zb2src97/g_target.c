...
*/
void Use_Target_Speaker(edict_t * ent, edict_t * other,
						edict_t * activator)
{
	int chan;
...
{
	if (ent->spawnflags & 1)
		strncpy(game.helpmessage1, ent->message,
				sizeof(game.helpmessage2) - 1);
	else
		strncpy(game.helpmessage2, ent->message,
				sizeof(game.helpmessage1) - 1);
...
void use_target_explosion(edict_t * self, edict_t * other,
						  edict_t * activator)
{
...
*/
void use_target_changelevel(edict_t * self, edict_t * other,
							edict_t * activator)
{
	...
	if (deathmatch->value) {
		if (activator && activator->client)
			gi.bprintf(PRINT_HIGH, "%s"S_COLOR_WHITE" exited the level.\n",
...
void use_target_splash(edict_t * self, edict_t * other,
					   edict_t * activator)
{
	gi.WriteByte(svc_temp_entity);
...
void use_target_spawner(edict_t * self, edict_t * other,
						edict_t * activator)
{
	edict_t *ent;
...
void use_target_blaster(edict_t * self, edict_t * other,
						edict_t * activator)
{
	int effect;
...
void target_laser_think(edict_t * self)
{
	...
	while (1) {
		...
		if ((tr.ent->takedamage) && !(tr.ent->flags & FL_IMMUNE_LASER))
			T_Damage(tr.ent, self, self->activator, self->movedir,
					 tr.endpos, vec3_origin, self->dmg, 1, DAMAGE_ENERGY,
					 MOD_TARGET_LASER);

		// if we hit something that's not a monster or player or is immune
...
void target_mal_laser_use(edict_t * self, edict_t * other,
						  edict_t * activator)
{
	self->activator = activator;
	if (self->spawnflags & 1)
...
void target_lightramp_use(edict_t * self, edict_t * other,
						  edict_t * activator)
{
	if (!self->enemy) {
		edict_t *e;
		...
		while (1) {
			...
			if (strcmp(e->classname, "light") != 0) {
				gi.dprintf("%s at %s ", self->classname,
						   vtos(self->s.origin));
...
void target_earthquake_use(edict_t * self, edict_t * other,
						   edict_t * activator)
{
