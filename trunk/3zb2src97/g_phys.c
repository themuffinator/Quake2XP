...
edict_t *SV_TestEntityPosition(edict_t * ent)
{
	...
	trace =
		gi.trace(ent->s.origin, ent->mins, ent->maxs, ent->s.origin, ent,
				 MASK_SOLID);
...
//PON-CTF
		if (!i /* ent->client */ ) {
			if (!ent->client->zc.trapped && !i) {
				numplanes = 0;
				if (ent->waterlevel
					|| (!ent->groundentity && ent->velocity[2] > 10))
					goto VELCX;
				i = 0;
...
//ponko
		if (check->classname[0] == 'R'
			&& (check->classname[6] == 'X' || check->classname[6] == '3'))
			continue;
...
// move origin
	...
	if (ent->classname[0] == 'R'
		&& (ent->classname[6] == 'X' || ent->classname[6] == '3')) {
		...
		VectorAdd(ent->union_ent->s.origin, ent->moveinfo.dir,
				  ent->s.origin);
	}
	...
	if (ent->velocity[2] || ent->velocity[1] || ent->velocity[0]) {
		...
		if (ent->svflags & SVF_MONSTER) {
			if (!deathmatch->value)
				...
				mask = MASK_BOTSOLIDX;	// MASK_PLAYERSOLID;
		} else
			mask = MASK_SOLID;
