...
qboolean Pickup_Weapon (edict_t * ent, edict_t * other) {
	...
	if ((other->svflags & SVF_MONSTER) && ctf->value
		&& other->client->zc.route_trace) {
	...
	if (other->svflags & SVF_MONSTER && (other->client->pers.weapon == Fdi_BLASTER	/* FindItem("blaster") */
		|| other->client->pers.weapon == Fdi_GRENADES /* FindItem("Grenades") */)) {
		ent->item->use (other, ent->item);	// other->client->pers.weapon
		// = ent->item;
		ShowGun (other);
	}
	return qtrue;
...
void ChangeWeapon (edict_t * ent) {
	...
	if (ctf->value == 2) {
		if (Q_stricmp
			(ent->client->pers.weapon->classname, "weapon_grapple") == 0) {
...
void NoAmmoWeaponChange (edict_t * ent) {
	...
	// RAFAEL
	else if (ent->client->pers.inventory[ITEM_INDEX (Fdi_MAGSLUGS	/* FindItem
																	   ("mag
																	   slug")
																	   */)]
																	   && ent->client->pers.inventory[ITEM_INDEX (Fdi_PHALANX	/* FindItem
																																 ("phalanx")
																																 */)]) {
		item = Fdi_PHALANX;		// FindItem ("phalanx");
	}
	// RAFAEL
	else if (ent->client->pers.inventory[ITEM_INDEX (Fdi_CELLS	/* FindItem
																   ("cells")
																   */)]
																   && ent->client->pers.inventory[ITEM_INDEX (Fdi_BOOMER	/* FindItem
																															 ("ionripper")
																															 */)]) {
		...
		//      return;
																  ...
																  */)] >
																  1 && ent->client->pers.inventory[ITEM_INDEX (Fdi_SUPERSHOTGUN	/* FindItem("super
																																  shotgun")
																																  */
																																  )]) {
...
void Weapon_Generic2 (edict_t * ent, int FRAME_ACTIVATE_LAST,
	int FRAME_FIRE_LAST, int FRAME_IDLE_LAST,
	int FRAME_DEACTIVATE_LAST, int *pause_frames,
		...
		for (n = 0; fire_frames[n]; n++) {
			if (ent->client->ps.gunframe == fire_frames[n]) {
				...
				if (ent->client->quad_framenum > level.framenum)
					...
					gi.soundindex ("items/damage3.wav"), 1,
					ATTN_NORM, 0);
				//ZOID
...
void weapon_grenade_fire (edict_t * ent, qboolean held) {
	...
	if (ent->deadflag || ent->s.modelindex != 255)	// VWep animations
		// screw up corpses
	{
		return;
	}
...
void Weapon_Grenade (edict_t * ent) {
	...
	if (ent->client->weaponstate == WEAPON_FIRING) {
		if (ent->client->ps.gunframe == 5)
			...
			gi.soundindex ("weapons/hgrena1b.wav"), 1, ATTN_NORM,
			0);

		if (ent->client->ps.gunframe == 11) {
			if (!ent->client->grenade_time) {
				ent->client->grenade_time =
					level.time + GRENADE_TIMER + 0.2;
...
======================================================================
*/
void fire_lockon_rocket (edict_t * self, vec3_t start, vec3_t dir,
...
void Weapon_RocketLauncher_Fire (edict_t * ent) {
	...
	if (ent->client->zc.aiming != 4)
		...
		damage -= 20;
		fire_lockon_rocket (ent, start, forward, damage, 20, damage_radius,
			radius_damage);
	}
...
void Weapon_LockonRocketLauncher_Fire (edict_t * ent) {
	...
	if (ent->client->buttons & BUTTON_ATTACK) {
		ent->client->zc.lockon = qfalse;
		if (ent->client->zc.aiming == 0) {
			gi.sound (ent, CHAN_WEAPON,
				gi.soundindex ("weapons/sshotr1b.wav"), 1, ATTN_NONE,
				0);
			ent->client->zc.aiming = 3;
			if (ent->client->zc.distance < 10
...
void Weapon_RocketLauncher (edict_t * ent) {
	...
	if (0 /* 1 */) {
		Weapon_Generic (ent, 4, 12, 50, 54, pause_frames, fire_frames,
			Weapon_LockonRocketLauncher_Fire);
	}
	else {
		Weapon_Generic (ent, 4, 12, 50, 54, pause_frames, fire_frames,
...
void Gatringgun_Fire (edict_t * ent) {
	...
	gi.sound (ent, CHAN_AUTO, gi.soundindex ("3zb/gatgf.wav"), u, ATTN_NORM,
		0);
...
======================================================================
*/
void fire_sniperail (edict_t * self, vec3_t start, vec3_t aimdir,
	int damage, int kick);
...
void RSight_think (edict_t * ent) {
	...
	if (ent->owner->client->zc.autozoom) {
		...
		if (f < 100)
			ent->owner->client->zc.distance = 90;
		...
		//      else if(f < 800) ent->owner->client->zc.distance = 45;
			...
			ent->owner->client->zc.distance = 15;

		if (ent->owner->client->ps.fov != ent->owner->client->zc.distance) {
			f = ent->owner->client->ps.fov -
				ent->owner->client->zc.distance;
			if (f > 15 || f < -15)
				gi.sound (ent->owner, CHAN_AUTO,
...
void Weapon_Ionripper (edict_t * ent) {
	...
	Weapon_Generic (ent, 4, 6, 36, 39, pause_frames, fire_frames,
...
void weapon_phalanx_fire (edict_t * ent) {
	...
	if (ent->client->ps.gunframe == 8) {
		...
		fire_plasma (ent, start, forward, damage, 725, damage_radius,
			radius_damage);
...
void Weapon_Phalanx (edict_t * ent) {
	...
	Weapon_Generic (ent, 5, 20, 58, 63, pause_frames, fire_frames,
		weapon_phalanx_fire);
...
void Weapon_Trap (edict_t * ent) {
	...
	if (ent->client->weaponstate == WEAPON_FIRING) {
		if (ent->client->ps.gunframe == 5)
			// RAFAEL 16-APR-98
			// gi.sound(ent, CHAN_WEAPON,
			// gi.soundindex("weapons/hgrena1b.wav"), 1, ATTN_NORM, 0);
			gi.sound (ent, CHAN_WEAPON,
			gi.soundindex ("weapons/trapcock.wav"), 1, ATTN_NORM,
			0);
		// END 16-APR-98
		...
		if (ent->client->ps.gunframe == 11) {
			if (!ent->client->grenade_time) {
				...
				// RAFAEL 16-APR-98
				ent->client->weapon_sound =
					gi.soundindex ("weapons/traploop.wav");
				// END 16-APR-98
			}
			// they waited too long, detonate it in their hand
			if (!ent->client->grenade_blew_up
