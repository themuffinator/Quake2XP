	...
	for (i = 0, item = itemlist; i < game.num_items; i++, item++) {
		...
		if (!strcmp (item->classname, ent->classname)) {	// found it
			...
			if (!strcmp (ent->classname, "weapon_shotgun"))
				mpindex[WEAP_SHOTGUN] = i;
			else if (!strcmp (ent->classname, "weapon_supershotgun"))
				mpindex[WEAP_SUPERSHOTGUN] = i;
			else if (!strcmp (ent->classname, "weapon_machinegun"))
				mpindex[WEAP_MACHINEGUN] = i;
			else if (!strcmp (ent->classname, "weapon_chaingun"))
				mpindex[WEAP_CHAINGUN] = i;
			else if (!strcmp (ent->classname, "ammo_grenades"))
				mpindex[WEAP_GRENADES] = i;
			else if (!strcmp (ent->classname, "weapon_grenadelauncher"))
				mpindex[WEAP_GRENADELAUNCHER] = i;
			else if (!strcmp (ent->classname, "weapon_rocketlauncher"))
				mpindex[WEAP_ROCKETLAUNCHER] = i;
			else if (!strcmp (ent->classname, "weapon_hyperblaster"))
				mpindex[WEAP_HYPERBLASTER] = i;
			else if (!strcmp (ent->classname, "weapon_boomer"))
				mpindex[WEAP_BOOMER] = i;
			else if (!strcmp (ent->classname, "weapon_railgun"))
				mpindex[WEAP_RAILGUN] = i;
			else if (!strcmp (ent->classname, "weapon_phalanx"))
				mpindex[WEAP_PHALANX] = i;
...
void ED_ParseField (char *key, char *value, edict_t * ent) {
	...
	for (f = fields; f->name; f++) {
		if (!Q_stricmp (f->name, key)) {	// found it
...
void G_FindTrainTeam () {
	...
	for (i = maxclients->value + 1; i < globals.num_edicts; i++, e++) {
		if (e->inuse && e->classname) {
			if (!Q_stricmp (e->classname, "path_corner") && e->targetname
				&& e->target) {
...
void G_FindRouteLink (edict_t * ent) {
	...
	for (i = 0; i < CurrentIndex; i++) {
		if (Route[i].state == GRS_NORMAL) {
			for (j = 0; j < CurrentIndex; j++) {
				...
				rs_trace =
					gi.trace (Route[j].Pt, NULL, NULL, Route[i].Pt, ent,
					MASK_SOLID);
				// found!!
	...
	if (ctf->value && bot_team_flag1 && bot_team_flag2) {
		...
		for (i = (CurrentIndex - 1); i >= 0; i--) {
			if (Route[i].state < GRS_ITEMS) {
				if (Route[i].state == GRS_REDFLAG
					|| Route[i].state == GRS_BLUEFLAG) {
					if (Route[i].ent == bot_team_flag1) {
						j = FOR_FLAG1;
						k = i;
					}
					else if (Route[i].ent == bot_team_flag2) {
						j = FOR_FLAG2;
						k = i;
					}
				}
...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint) {
	...
	while (1) {
		...
		if (!entities)
			break;
		if (com_token[0] != '{')
			gi.error ("ED_LoadFromFile: found %s when expecting {",
			com_token);
		...
		if (Q_stricmp (ent->classname, "target_laser") == 0) {
			if (laser < MAX_LASERINDEX)
				LaserIndex[laser++] = ent;
		}
		//PON-CTF
...
// cursor positioning
xl < value >
xr < value > yb < value > yt < value > xv < value > yv < value >
// drawing
statpic < name >
pic < stat > num < fieldwidth > <stat > string < stat >
// control
	if <stat
	...
	*/
	void SP_worldspawn (edict_t * ent) {
		...
		snd_fry = gi.soundindex ("player/fry.wav");	// standing in lava /
		// slime
		...
		gi.soundindex ("player/gasp2.wav");	// head breaking surface, not
		// gasping
