...
void M_CheckGround (edict_t * ent) {
	...
	trace =
		gi.trace (ent->s.origin, ent->mins, ent->maxs, point, ent,
		MASK_BOTSOLID
		/* CONTENTS_SOLID|CONTENTS_WINDOW|CONTENTS_MONSTER */);
	...
	if ( /* trace.ent && */ ( /* trace.startsolid || */ trace.allsolid)) {
		if (1
			/* trace.ent->classname[0] == 'f' && trace.ent->classname[5]
			   == 'r' */
			   ) {
			...
			tracep = gi.trace (stp, v1, v2, point, ent, MASK_BOTSOLID	/* MASK_PLAYERSOLID
																		 */);
			if (tracep.ent
				&& !tracep.allsolid /* && !tracep.startsolid */) {
				if (tracep.ent->classname[0] == 'f'	/* &&
													   tracep.ent->classname[5]
													   == 'r' */) {
					VectorCopy (tracep.endpos, ent->s.origin);
					//                  gi.bprintf(PRINT_HIGH,"ogee done\n");
					ent->groundentity = tracep.ent;
					/* if(tracep.ent->classname[5] == 'r') */ ent->
						groundentity_linkcount = tracep.ent->linkcount;
					// ent->velocity[2] = 0;
					gi.linkentity (ent);
					return;
				}
...
void M_droptofloor (edict_t * ent) {
	...
	trace =
		gi.trace (ent->s.origin, ent->mins, ent->maxs, end, ent,
		MASK_MONSTERSOLID);
