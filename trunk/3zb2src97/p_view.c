...
void P_DamageFeedback(edict_t * player)
{
	...
	count =
		(client->damage_blood + client->damage_armor +
	...
	if ((level.time > player->pain_debounce_time)
		...
		gi.sound(player, CHAN_VOICE,
				 gi.soundindex(va("*pain%i_%i.wav", l, r)), 1, ATTN_NORM,
				 0);
	}
	...
	if (client->damage_parmor)
		VectorMA(v, (float) client->damage_parmor / realcount, power_color,
				 v);
	...
	if (kick && player->health > 0)	// kick of 0 means no view adjust at
									// all
	{
...
void SV_CalcGunOffset(edict_t * ent)
{
	...
	for (i = 0; i < 3; i++) {
		delta =
			ent->client->oldviewangles[i] - ent->client->ps.viewangles[i];
		if (delta > 180)
			delta -= 360;
		if (delta < -180)
			delta += 360;
...
void SV_CalcBlend(edict_t * ent)
{
	...
	// dont drop radial blur flag for died player
	...
	// RAFAEL
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM, gi.soundindex("items/airout.wav"), 1,
					 ATTN_NORM, 0);
		if (remaining > 30 || (remaining & 4))
			SV_AddBlend(0, 1, 0, 0.1, ent->client->ps.blend);
	...
	if (ent->client->bonus_alpha > 0)
		...
		if(!r_radialBlur->value)
		SV_AddBlend(0.85, 0.7, 0.2, ent->client->bonus_alpha, ent->client->ps.blend);
...
void P_WorldEffects(void)
{
	...
	//
	if (old_waterlevel == 3 && waterlevel != 3) {
		if (current_player->air_finished < level.time) {	// gasp for
															// air
			...
			PlayerNoise(current_player, current_player->s.origin,
						PNOISE_SELF);
		} else if (current_player->air_finished < level.time + 11) {	// just
																		// break
																		// surface
			gi.sound(current_player, CHAN_VOICE,
			...
			if (((int) (current_client->breather_framenum - level.framenum)
				...
				if (!current_client->breather_sound)
					gi.sound(current_player, CHAN_AUTO,
							 gi.soundindex("player/u_breath1.wav"), 1,
							 ATTN_NORM, 0);
				else
					gi.sound(current_player, CHAN_AUTO,
							 gi.soundindex("player/u_breath2.wav"), 1,
							 ATTN_NORM, 0);
		...
		if (current_player->air_finished < level.time) {	// drown!
			if (current_player->client->next_drown_time < level.time
				...
				if (current_player->health <= current_player->dmg)
					gi.sound(current_player, CHAN_VOICE,
							 gi.soundindex("player/drown1.wav"), 1,
							 ATTN_NORM, 0);
				else if (rand() & 1)
					gi.sound(current_player, CHAN_VOICE,
	...
	if (waterlevel
		...
		if (current_player->watertype & CONTENTS_LAVA) {
			if (current_player->health > 0
				...
				if (rand() & 1)
					gi.sound(current_player, CHAN_VOICE,
							 gi.soundindex("player/burn1.wav"), 1,
							 ATTN_NORM, 0);
				else
					gi.sound(current_player, CHAN_VOICE,
							 gi.soundindex("player/burn2.wav"), 1,
							 ATTN_NORM, 0);
...
//ZOID
		...
		if (remaining == 30 && (ent->svflags & SVF_MONSTER))	// beginning
																// to fade
			gi.sound(ent, CHAN_ITEM, gi.soundindex("items/damage2.wav"), 1,
					 ATTN_NORM, 0);
	}
...
void G_SetClientEffects (edict_t *ent)
{
	...
	ent->s.renderfx |= (RF_SHELL_RED|RF_SHELL_GREEN|RF_SHELL_BLUE|RF_SHELL_GOD);
...
void ClientEndServerFrame(edict_t * ent)
{
	...
	//
	xyspeed =
		sqrt(ent->velocity[0] * ent->velocity[0] +
			 ent->velocity[1] * ent->velocity[1]);

	if (xyspeed < 5) {
		bobmove = 0;
		current_client->bobtime = 0;	// start at beginning of cycle
										// again
	} else if (ent->groundentity) {	// so bobbing only cycles when on
									// ground
		if (xyspeed > 210)
			bobmove = 0.25;
		else if (xyspeed > 100)
...
//  SV_CalcBlend (ent);
	...
	if (ent->deadflag) {
		...
		if (ent->s.modelindex != skullindex
			&& ent->s.modelindex != headindex) {
