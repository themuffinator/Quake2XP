...
qboolean Pickup_Navi(edict_t * ent, edict_t * other)
{
	...
	if (ent->classname[6] == '3' && ent->union_ent) {
		if (ent->target_ent ==
			other /*->client->zc.second_target == ent*/ ) {
//gi.bprintf(PRINT_HIGH,"get target!\n");
			...
			for (i = -MAX_DOORSEARCH; i < MAX_DOORSEARCH; i++) {
				if (i <= 0)
					j = other->client->zc.routeindex - (MAX_DOORSEARCH -
														i);
				else
...
//else gi.bprintf(PRINT_HIGH,"not target!\n");
	...
	// roamnavi
		...
		for (i = 0; i < 10; i++) {
			...
			if (!Route[other->client->zc.routeindex + i].index)
				break;
			if (Route[other->client->zc.routeindex + i].state ==
				GRS_PUSHBUTTON
				&& Route[other->client->zc.routeindex + i].ent ==
				ent->union_ent) {
	...
	if (ctf->value && ent->classname[6] == 'F') {
		if (other->client->resp.ctf_team == CTF_TEAM1) {
			...
			if (other->moveinfo.state == CARRIER
				|| other->moveinfo.state == SUPPORTER) {
				if (ent->owner != NULL)
					other->target_ent = ent->owner;
				...
				if (flage->solid == SOLID_NOT && flagf->solid != SOLID_NOT) {
					...
					if (ent->target_ent != NULL)
						other->target_ent = ent->target_ent;
				}
...
void Use_QuadFire(edict_t * ent, gitem_t * item)
{
	...
	gi.sound(ent, CHAN_ITEM, gi.soundindex("items/quadfire1.wav"), 1,
			 ATTN_NORM, 0);
}
...
void Use_Invulnerability(edict_t * ent, gitem_t * item)
{
	...
	gi.sound(ent, CHAN_ITEM, gi.soundindex("items/protect.wav"), 1,
			 ATTN_NORM, 0);
}
...
qboolean Pickup_Armor(edict_t * ent, edict_t * other)
{
	...
	// use the better armor
		...
		if (newinfo->normal_protection > oldinfo->normal_protection) {
			...
			salvage =
				oldinfo->normal_protection / newinfo->normal_protection;
			salvagecount =
...
void Use_PowerArmor(edict_t * ent, gitem_t * item)
{
	...
	if (ent->flags & FL_POWER_ARMOR) {
		...
		gi.sound(ent, CHAN_ITEM, gi.soundindex("misc/power2.wav"), 1, ATTN_NORM, 0);	// FIXME
																						// powering
																						// down
																						// sound
	} else {
		...
		ent->flags |= FL_POWER_ARMOR;
		gi.sound(ent, CHAN_ITEM, gi.soundindex("misc/power1.wav"), 1, ATTN_NORM, 0);	// FIXME
																						// powering
																						// up
																						// sound
	}
		...
		if (ent->item->use) {
			...
			if (k > WEAP_BLASTER) {
				if (Bot[other->client->zc.botindex].param[BOP_PRIWEP] == k)
					...
					if (Bot[other->client->zc.botindex].
						param[BOP_SECWEP] == k)
						ent->item->use(other, ent->item);
				}
			}
...
//  else gi.bprintf(PRINT_HIGH,"get %s %x inv %i!\n",ent->classname,ent->spawnflags,other->client->pers.inventory[ITEM_INDEX(ent->item)]);
	...
	if (!k && chedit->value && CurrentIndex < MAXNODES
		...
		if ((ent->classname[0] == 'w'
			 || (ent->classname[0] == 'i'
				 && (ent->classname[5] == 'q' || ent->classname[5] == 't'
					 || ent->classname[5] == 'f'
					 || ent->classname[5] == 'i'
					 || ent->classname[5] == 'p'
					 || ent->classname[5] == 's'
					 ...
					 || ent->classname[5] == 'e'
					 || ent->classname[5] == 'a'))
			 || (ent->classname[0] == 'i' && ent->classname[5] == 'h'
				 && ent->classname[12] == 'm')
			 || (ent->classname[0] == 'a')
			)
...
//          gi.bprintf(PRINT_HIGH,"woohoo!\n");
			VectorCopy( /* ent->s.origin */ ent->monsterinfo.last_sighting,
					   Route[CurrentIndex].Pt);
			Route[CurrentIndex].ent = ent;
			if (ent == bot_team_flag1) {
				Route[CurrentIndex].state = GRS_REDFLAG;	/* gi.bprintf(PRINT_HIGH,"woohoo!\n"); */
			} else if (ent == bot_team_flag2) {
				Route[CurrentIndex].state = GRS_BLUEFLAG;	/* gi.bprintf(PRINT_HIGH,"woohoo!\n"); */
			} else
...
static void drop_temp_touch(edict_t * ent, edict_t * other,
...
//  if(ctf->value && chedit->value) ent->moveinfo.speed = 0;
	...
	if (ent->classname[0] == 'w' || ent->classname[0] == 'i'
		|| ent->classname[0] == 'a') {
		k = 0;
		...
		for (i = 0; i < 8; i++) {
			if (i < 4) {
				...
				for (j = 32; j < 100; j += 2) {
					...
					trmax[2] -= 128;
					tr = gi.trace(trmin, mins, maxs, trmax, ent,
								  MASK_PLAYERSOLID);
					trx =
						gi.trace(trmin, mins, maxs, trmax, ent,
								 CONTENTS_WATER);
					VectorCopy(tr.endpos, trmax);
					// trmax[2] += 16;
					if (0		/* (trmin[2] - trx.endpos[2]) <= 39
								   &&ent->classname[0] == 'w' && k == 0
								   && !(gi.pointcontents (trmin) &
								   CONTENTS_WATER) && (gi.pointcontents
								   (trmax) & CONTENTS_WATER) */
						) {
						it = FindItem("Roam Navi");
						it_ent = G_Spawn();
						it_ent->classname = it->classname;
						trmin[0] =
							ent->s.origin[0] + cos(yaw) * (j + trmin[2] -
														   tr.endpos[2] +
														   100);
						trmin[1] =
							ent->s.origin[1] + sin(yaw) * (j + trmin[2] -
														   tr.endpos[2] +
														   100);
						trmin[2] = trmax[2] + 8;
						trx =
							gi.trace(trmax, mins, maxs, trmin, ent,
									 CONTENTS_WATER);
				...
				for (j = 32; j < 80; j += 2) {
					...
					VectorCopy(trmin, trmax);
					trmax[2] -= 128;
					tr = gi.trace(trmin, NULL, NULL, trmax, ent,
								  MASK_PLAYERSOLID);
					if (tr.endpos[2] < ent->s.origin[2] - 16
						&& tr.endpos[2] > min[2] && !tr.allsolid
						&& !tr.startsolid) {
...
//      dest[1] = (ent->union_ent->s.origin[1] + ent->union_ent->mins[1] + ent->union_ent->s.origin[1] + ent->union_ent->maxs[1])/2;
		...
		dest[0] = (ent->union_ent->s.origin[0] + ent->union_ent->mins[0] + ent->union_ent->s.origin[0] + ent->union_ent->maxs[0]) / 2;	// ent->s.origin[0];
		dest[1] =
			(ent->union_ent->s.origin[1] + ent->union_ent->mins[1] +
			 ent->union_ent->s.origin[1] + ent->union_ent->maxs[1]) / 2;
		...
		for (i = ent->union_ent->s.origin[2] + ent->union_ent->mins[2]	/* moveinfo.start_origin[2]+15
			...
			if ((!tr.startsolid && !tr.allsolid) && j == 1) {
				j = 2;
				break;
			} else if ((tr.startsolid || tr.allsolid) && j == 0
					   && tr.ent == ent->union_ent)
				j = 1;

		}
		VectorCopy(dest, ent->s.origin);
		VectorSubtract(ent->s.origin, ent->union_ent->s.origin,
					   ent->moveinfo.dir);
	}
//////////
/*	if (ent->model)
...
//ent->s.modelindex =gi.modelindex ("models/items/armor/body/tris.md2");
	...
	tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, dest, ent,
				  MASK_SOLID);
	if (tr.startsolid && ent->classname[0] != 'R'
		&& ent->classname[6] != 'X') {
//      gi.dprintf ("droptofloor: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin));
		G_FreeEdict(ent);
		return;
...
void PrecacheItem(gitem_t * it)
{
	...
	while (*s) {
		...
		if (len >= MAX_QPATH || len < 5)
			gi.error("PrecacheItem: %s has bad precache string",
					 it->classname);
...
void ZIGFlagThink(edict_t * ent)
{
	...
	if (count > 4) {
		...
		for (i = maxclients->value + 1; i < globals.num_edicts; i++) {
			tre = &g_edicts[i];
			if (!tre->inuse || tre->deadflag)
				continue;

			if (tre->classname[0] == 'p'
				&& tre->movetype != MOVETYPE_NOCLIP && tre->client) {
				if (tre->client->zc.second_target)
					continue;
				VectorSubtract(tre->s.origin, ent->s.origin, v);
				if (VectorLength(v) < 350 && Bot_traceS(ent, tre)
					&& v[2] < -JumpMax) {
...
//ZOID
	...
	ent->item = item;
	ent->nextthink = level.time + 2 * FRAMETIME;	// items start after
													// other solids
...
*/
	{
	 ...
	 CTFPickup_Flag,
	 NULL,
	 CTFDrop_Flag,				// Should this be null if we don't want
								// players to drop it manually?
	 NULL,
	 "ctf/flagtk.wav",
...
*/
	{
	 ...
	 ZIGPickup_Flag,
	 NULL,
	 ZIGDrop_Flag,				// Should this be null if we don't want
								// players to drop it manually?
	 NULL,
	 "3zb/emgcall.wav",
...
/* Resistance Tech */
	{
	 ...
	 CTFPickup_Tech,
	 NULL,
	 CTFDrop_Tech,				// Should this be null if we don't want
								// players to drop it manually?
	 NULL,
	 "items/pkup.wav",
