...
void CleanDuplicateLights(void){
	...
	for(light2 = light1->s_next; light2; light2 = plug) {
		...
		if ((VectorLength(tmp) < r_lightsWeldThreshold->value) || VectorCompare(light1->origin, light2->origin)){
...
qboolean R_DrawLightOccluders(worldShadowLight_t *light)
{
	...
	if(BoundsAndSphereIntersect (light->mins, light->maxs, r_origin, 25))
		return true;
	}
	...
	if(BoundsAndSphereIntersect (light->mins, light->maxs, r_origin, 25))				// player in light bounds
