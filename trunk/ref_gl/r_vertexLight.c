...
int RecursiveLightPoint (mnode_t * node, vec3_t start, vec3_t end) {
	const float ambientScale = max(r_lightmapScale->value, 0.33f) * (1.f / 255.f);
	float front, back, f;
	int side, smax;
	cplane_t *plane;
	vec3_t mid;
	...
	for (i = 0, surf = &r_worldmodel->surfaces[node->firstsurface]; i < node->numsurfaces; i++, surf++) {
		...
		if (r_worldmodel->useXPLM) {
			int tmax = (surf->extents[1] / (int)r_worldmodel->lightmap_scale) + 1;
			int size = smax * tmax * 3;

			for (int j = 0; j < XPLM_NUMVECS; j++, lm += size) {
