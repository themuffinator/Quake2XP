...
/*=============================================================
 WriteScaledDir - Berserker's non normalized vector compression
 =============================================================*/
...
void NetThrowGibs (edict_t *self, int damage) {
	...
	gi.WriteByte (svc_temp_entity);
	gi.WriteByte (TE_GIB_CLIENT);

	gi.WritePosition (self->absmin);     ///self->s.origin
	...
	ClipGibVelocity (&gib);
	WriteScaledDir (gib.velocity);          // send the 5 bytes instead 12
...
void SP_misc_explobox (edict_t *self) {
	...
	self->collision_model = TR_Model_Get("explobox", &self->collision_model_index);
