...
//	mod_GetLeadoffVec(ent, start, 2048, 1000, true, firevec);
...
void Weapon_HyperBlaster_Fire (edict_t *ent) {
	...
	if (ent->client->ps.gunframe == 12) {
		gi.sound (ent, CHAN_AUTO, gi.soundindex ("weapons/hyprbd1a.wav"), 0.7f, ATTN_NORM, 0);
		ent->client->weapon_sound = 0;
	}
...
void Extr_Shell (edict_t *self, char *modelname, vec3_t start, vec3_t aimdir, int speed, vec3_t velocity, char *kierunek) {
	...
	if (deathmatch->value || coop->value)
		return; // luski nie bedzie w deathmatchu i w coop.
...
void Chaingun_Fire (edict_t *ent) {
	...
	int			damage;
	int			kick = 2;
	vec3_t start_shell, forward_shell, left_shell, offset_shell;
	if (deathmatch->value)
		damage = 6;
	else
		damage = 8;
	...
	VectorSet (offset_shell, 12, 8.7, ent->viewheight - 3.2);
	AngleVectors (ent->client->v_angle, forward_shell, left_shell, NULL);
	P_ProjectSource (ent, offset_shell, forward_shell, left_shell, start_shell);
	VectorScale (forward_shell, 2, ent->client->kick_origin);
	Extr_Shell (ent, "models/shells/m_shell.md2", start_shell, forward_shell, 2, ent->velocity, "left");
...
void weapon_shotgun_fire (edict_t *ent) {
	...
	P_ProjectSource (ent, offset_shell, forward_shell, left_shell, start_shell);
	VectorScale (forward_shell, -2, ent->client->kick_origin);
	Extr_Shell (ent, "models/shells/s_shell.md2", start_shell, forward_shell, 2, ent->velocity, "right");
...
void weapon_supershotgun_fire (edict_t *ent) {
	...
	VectorSet (offset_shell, 15, 10.9, ent->viewheight - 5.4);
	AngleVectors (ent->client->v_angle, forward_shell, left_shell, NULL);
	P_ProjectSource (ent, offset_shell, forward_shell, left_shell, start_shell);
	VectorScale (forward_shell, 0, ent->client->kick_origin);
	Extr_Shell (ent, "models/shells/s_shell.md2", start_shell, forward_shell, 4, ent->velocity, "right");
