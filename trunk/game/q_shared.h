...
//#define	MAX_EDICTS			8196	// pax mission pak optimisation
...
#define MASK_HEATHAZE			(CONTENTS_LAVA)
// Q2XP added
#define     MASK_VISIBILILITY         (CONTENTS_SOLID/*|CONTENTS_WINDOW*/|CONTENTS_WATER|CONTENTS_LAVA|CONTENTS_SLIME)

#define	MASK_WATERONLY				(CONTENTS_WATER)
#define	MASK_LAVA				(CONTENTS_LAVA)
#define	MASK_SLIME				(CONTENTS_SLIME)
...
// sound attenuation values (by willow)
...
#define CS_GENERAL		(CS_LIGHTFLARES+MAX_LIGHTFLARES)
...
#define PARTICLE_UNDERWATER				256
#define PARTICLE_OVERBRIGHT				512
#define PARTICLE_SPIRAL					1024
#define PARTICLE_AIRONLY				2048
#define PARTICLE_LIGHTING				4096
#define PARTICLE_ALIGNED				8192
#define PARTICLE_NONSOLID				16384
#define PARTICLE_STOPED					32768
#define PARTICLE_CLAMP					65536
...
typedef enum {
	PF_NOERRORS			= BIT(0),
	PF_NOWARNINGS			= BIT(1),
	PF_ALLOWPATHNAMES		= BIT(2)	// needed for eg. correct number division parsing
} parserFlags_t;

typedef enum {
	TT_BAD,
	TT_STRING,					// unquoted string
	TT_NUMBER,
	TT_PUNCTUATION
} tokenType_t;

typedef struct {
	int			line;			// the line index on which the token is situated
...
typedef struct {
	...
	token_t		token;			// saved token, for UngetToken()

	const char 	*text;
	int			line;		// current parse line
