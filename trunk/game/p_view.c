...
void P_DamageFeedback (edict_t *player) {
	...
	if ((level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum)) {
		...
		if (player->health < 25)
			...
			l = 100;
		gi.sound (player, CHAN_VOICE, gi.soundindex (va ("*pain%i_%i.wav", l, r)), 0.65, ATTN_NORM, 0);
	}
...
void P_WorldEffects (void) {
	...
	if (old_waterlevel && !waterlevel) {
		PlayerNoise (current_player, current_player->s.origin, PNOISE_SELF);
		gi.sound (current_player, CHAN_BODY, gi.soundindex ("player/watr_out.wav"), 0.65, ATTN_NORM, 0);
		current_player->flags &= ~FL_INWATER;
	}
	...
	if (old_waterlevel != 3 && waterlevel == 3) {
		gi.sound (current_player, CHAN_BODY, gi.soundindex ("player/watr_un.wav"), 0.65, ATTN_NORM, 0);
	}
...
void G_SetClientSound (edict_t *ent) {
	...
	if (ent->client->pers.helpchanged && ent->client->pers.helpchanged <= 3 && !(level.framenum & 63)) {
		...
		//willow: very gentle volume level
		gi.sound (ent, CHAN_VOICE, gi.soundindex ("misc/pc_up.wav"), 0.25, ATTN_STATIC, 0);
	}
...
void ClientEndServerFrame (edict_t *ent) {
	...
	if (ent->client->v_angle[PITCH] > 180)
		ent->s.angles[PITCH] = (-360 + ent->client->v_angle[PITCH])*0.33333333;
	else
		ent->s.angles[PITCH] = ent->client->v_angle[PITCH] * 0.33333333;
