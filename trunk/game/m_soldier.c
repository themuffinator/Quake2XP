...
void soldier_step(edict_t *self) {
	...
	if (n == 0)
		...
		gi.sound(self, CHAN_BODY, sound_step4, 1, ATTN_FOOTSTEPS, 0);
}
...
void soldier_pain (edict_t *self, edict_t *other, float kick, int damage) {
	...
	n = self->s.skinnum | 1;
	if (n == 1)
		gi.sound (self, CHAN_VOICE, sound_pain_light, 1, ATTN_MEDIUM, 0);
	else if (n == 3)
		gi.sound (self, CHAN_VOICE, sound_pain, 1, ATTN_MEDIUM, 0);
	else
		gi.sound (self, CHAN_VOICE, sound_pain_ss, 1, ATTN_MEDIUM, 0);
...
void NetThrowGibs (edict_t *self, int damage);
...
void soldier_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point) {
	...
	if (self->s.skinnum == 1)
		gi.sound (self, CHAN_VOICE, sound_death_light, 1, ATTN_MEDIUM, 0);
	else if (self->s.skinnum == 3)
		gi.sound (self, CHAN_VOICE, sound_death, 1, ATTN_MEDIUM, 0);
	else // (self->s.skinnum == 5)
		gi.sound (self, CHAN_VOICE, sound_death_ss, 1, ATTN_MEDIUM, 0);
